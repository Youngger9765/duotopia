# Duotopia 11月交付功能 PRD (Product Requirements Document)

**版本**: 1.0
**日期**: 2025-11-17
**狀態**: ✅ Final 版本

---

## 一、交付概述

### 1.1 交付時程
- **開發月份**: 2025年11月
- **交付日期**: 2025年11月30日
- **階段**: Phase 1.5 - 訂閱與管理功能增強

### 1.2 交付目標
本次交付專注於完善訂閱管理系統、後台管理功能、以及全站國際化支援，為平台商業化運營奠定基礎。

---

## 二、核心功能交付清單

### 2.1 Admin 訂閱管理 - Trial Plan 顯示

#### 功能描述
確保 `/admin/subscription` 頁面正確顯示新註冊用戶的 Trial 訂閱資訊。

#### 需求
1. **Plan 欄位顯示**
   - 新註冊用戶應顯示 "30-Day Trial"
   - 已付費用戶顯示對應方案名稱（Tutor Teachers / School Teachers）

2. **End Date 欄位顯示**
   - Trial 用戶：顯示註冊日期 + 30 天
   - 付費用戶：顯示訂閱到期日
   - 格式：YYYY-MM-DD

#### 驗收標準
- [x] 新註冊用戶在 admin/subscription 頁面 Plan 欄位顯示 "30-Day Trial"
- [x] Trial 用戶的 End Date 正確顯示（註冊日 + 30天）
- [x] 付費用戶的 Plan 和 End Date 正確顯示

#### 實作記錄
- **完成日期**: 2025-11-18
- **修改檔案**:
  - `services/email_service.py:445` - 修改 `plan_name="30-Day Trial"`
  - `models.py:299` - 修改 `subscription_auto_renew` default=False（新用戶預設不啟用自動續訂）
  - `tests/integration/api/test_trial_plan_display.py` - 新增 TDD 測試
- **測試**: 2 passed (test_email_verification_creates_trial_plan, test_trial_plan_name_and_duration)
- **額外修復**: 新註冊用戶預設 `subscription_auto_renew=False`（沒綁卡不應啟用續訂）

---

### 2.2 退款機制（公司管理者後台）

#### 功能描述
為公司管理者提供完整的退款管理功能，支援全額退款操作與記錄追蹤。

#### 核心功能
1. **退款操作介面**
   - 搜尋交易記錄
   - 選擇退款項目
   - 輸入退款原因
   - 執行退款操作
   - 退款確認機制

2. **退款狀態管理**
   - 申請中 (PENDING)
   - 處理中 (PROCESSING)
   - 成功 (SUCCESS)
   - 失敗 (FAILED)
   - 已取消 (CANCELLED)

3. **訊息與通知**
   - 退款成功通知（Email + 系統通知）
   - 退款失敗通知（顯示失敗原因）
   - 管理者操作記錄

#### 技術實作
- **API 路由**: `POST /api/admin/refunds`
- **資料模型**:
  ```typescript
  interface Refund {
    id: string;
    transaction_id: string;
    amount: number;
    reason: string;
    status: RefundStatus;
    processed_by: number; // admin user id
    processed_at?: Date;
    error_message?: string;
  }
  ```
- **TapPay 整合**: 呼叫 TapPay Refund API
- **記錄稽核**: 完整記錄退款操作歷史

#### 驗收標準
- [ ] 管理者可搜尋並查看所有交易記錄
- [ ] 執行全額退款功能正常
- [ ] 退款成功後更新訂閱狀態
- [ ] 退款失敗顯示明確錯誤訊息
- [ ] 所有退款操作完整記錄
- [ ] Email 通知正確發送

---

### 2.3 試用點數轉移機制 (Trial Quota Transfer)

> **✅ 已完成 (2025-11-18)** - 升級時保留 Trial 剩餘點數

#### 功能描述
當用戶從 Trial 升級到付費方案時，自動將剩餘的試用點數轉移到新訂閱，避免點數浪費。

#### 業務規則
- **Trial 配額**: 10,000 點（30天）
- **付費配額**: Tutor 10,000 / School 25,000
- **轉移邏輯**: 付費配額 + Trial 剩餘 = 總配額

**範例**:
```
Trial 已用 3,000，剩餘 7,000
升級 School Teachers (25,000)
→ 總配額 = 25,000 + 7,000 = 32,000
```

#### 驗收標準
- [x] ✅ Trial 剩餘點數正確轉移
- [x] ✅ 前端顯示正確總配額
- [x] ✅ Admin 編輯時保留轉移點數
- [x] ✅ 測試通過 (4/4)
- [x] ✅ 已部署 Production

#### 實作記錄
- **完成日期**: 2025-11-18
- **測試**: 4 scenarios PASSED
- **部署**: ✅ main branch

---

### 2.4 後台帳款明細下載

#### 功能描述
提供管理端完整的帳款明細匯出功能，支援多種格式下載。

#### 核心功能
1. **帳款資料篩選**
   - 時間範圍選擇（起迄日期）
   - 訂閱方案篩選（Trial/Tutor/School）
   - 交易狀態篩選（成功/退款/失敗）
   - 用戶搜尋（姓名/Email）

2. **匯出格式**
   - **CSV**: 相容 Excel，適合數據分析
   - **XLSX**: Excel 原生格式，支援多工作表
   - 未來擴充: PDF 報表

3. **匯出欄位**
   | 欄位名稱 | 說明 |
   |---------|------|
   | 交易ID | Transaction ID |
   | 交易日期 | Transaction Date |
   | 用戶姓名 | Teacher Name |
   | 用戶Email | Teacher Email |
   | 訂閱方案 | Subscription Tier |
   | 交易金額 | Amount (NT$) |
   | 支付方式 | Payment Method |
   | 交易狀態 | Status |
   | 配額點數 | Credits Allocated |
   | 備註 | Notes |

#### 技術實作
- **API 路由**: `GET /api/admin/transactions/export`
- **Query 參數**:
  ```typescript
  interface ExportQuery {
    start_date: string;
    end_date: string;
    tier?: 'trial' | 'tutor' | 'school';
    status?: 'success' | 'refunded' | 'failed';
    format: 'csv' | 'xlsx';
  }
  ```
- **後端處理**:
  - CSV: Python `csv` module
  - XLSX: `openpyxl` library
  - 檔案命名: `transactions_YYYYMMDD_HHMMSS.{csv|xlsx}`

#### 驗收標準
- [x] ✅ 可選擇日期範圍匯出
- [x] ✅ CSV 格式正確（UTF-8 BOM，Excel 可開啟）
- [x] ✅ XLSX 格式正確（含標題列）
- [x] ✅ 篩選條件正確套用
- [x] ✅ 大量資料匯出不超時（測試 10,000 筆）
- [x] ✅ 下載檔名包含時間戳記
- [x] ✅ 中文欄位名稱正確顯示

#### ✅ 實作完成 (2025-11-17)
- **Commit**: `4ef9dd2`
- **功能**:
  - ✅ Subscriptions Tab - 下載訂閱資料
  - ✅ Transactions Tab - 下載交易記錄
  - ✅ Learning Tab - 下載學習分析
  - ✅ UTF-8 BOM 支援 Excel 中文顯示
  - ✅ 檔名自動加上日期 (如 `subscriptions_2025-11-17.csv`)

---

### 2.5 全站國際化 (i18n)

#### 功能描述
建立完整的多語言支援架構，實作中英文雙語介面。

#### 支援語言
- **中文 (zh)**: 繁體中文（預設）
- **英文 (en)**: 英文

#### i18n 架構

##### 前端實作
- **框架**: `react-i18next`
- **語系檔位置**: `/frontend/src/locales/`
  - `zh.json` - 繁體中文
  - `en.json` - 英文
- **語系切換**: Header 右上角語言選單
- **持久化**: LocalStorage 儲存使用者偏好

##### 後端實作
- **框架**: `babel` 或自定義 i18n service
- **語系檔位置**: `/backend/locales/`
  - `zh.json` - 繁體中文
  - `en.json` - 英文
- **Email 模板**: 支援多語言信件內容

#### 翻譯範圍

##### 前端全頁面文案
- [ ] 登入/註冊頁面
- [ ] 教師儀表板
- [ ] 班級管理頁面
- [ ] 學生管理頁面
- [ ] 課程管理頁面
- [ ] 作業管理頁面
- [ ] 批改介面
- [ ] 訂閱管理頁面
- [ ] 個人設定頁面
- [ ] 錯誤訊息
- [ ] 按鈕文字
- [ ] 表單標籤

##### 後台全模組文案
- [ ] 管理者儀表板
- [ ] 用戶管理
- [ ] 交易管理
- [ ] 退款管理
- [ ] 系統設定
- [ ] API 錯誤訊息
- [ ] Email 通知模板

#### 語系檔結構
```json
{
  "common": {
    "save": "儲存",
    "cancel": "取消",
    "delete": "刪除",
    "edit": "編輯"
  },
  "auth": {
    "login": "登入",
    "logout": "登出",
    "register": "註冊"
  },
  "dashboard": {
    "title": "儀表板",
    "welcome": "歡迎回來"
  },
  "subscription": {
    "trial": "試用中",
    "tutor": "Tutor 方案",
    "school": "School 方案"
  }
}
```

#### Fallback 機制
- **順序**: 使用者偏好語言 → 瀏覽器語言 → 預設中文
- **缺失翻譯**: 顯示 key 值（開發模式）或預設中文（生產模式）
- **動態載入**: 語系檔按需載入，減少初始載入時間

#### 技術實作
- **語言檢測**:
  ```typescript
  const detectLanguage = (): Language => {
    const stored = localStorage.getItem('language');
    if (stored) return stored as Language;

    const browser = navigator.language;
    return browser.startsWith('zh') ? 'zh' : 'en';
  };
  ```
- **Context Provider**:
  ```typescript
  <I18nextProvider i18n={i18n}>
    <App />
  </I18nextProvider>
  ```
- **使用範例**:
  ```typescript
  const { t } = useTranslation();
  return <h1>{t('dashboard.welcome')}</h1>;
  ```

#### 驗收標準
- [ ] 語系切換功能正常運作
- [ ] 切換語系後頁面即時更新
- [ ] 所有前台頁面文案已翻譯
- [ ] 所有後台頁面文案已翻譯
- [ ] Email 通知支援多語言
- [ ] API 錯誤訊息支援多語言
- [ ] 語系偏好正確儲存
- [ ] 重新整理頁面語系不變
- [ ] 無翻譯缺失（100% 覆蓋率）
- [ ] RTL 語言支援（預留，暫不實作）

---

## 三、資料模型變更

### 3.1 Teacher 模型擴充
```python
class Teacher(Base):
    # 原有欄位...

    # 新增欄位
    subscription_tier: str  # 'trial' | 'tutor' | 'school'
    trial_credits_remaining: int = 1000
    subscription_credits_remaining: int = 0
    subscription_credits_total: int = 0
    credits_reset_at: DateTime
    preferred_language: str = 'zh'  # 'zh' | 'en'
```

### 3.2 新增 Refund 模型
```python
class Refund(Base):
    __tablename__ = "refunds"

    id: int (Primary Key)
    transaction_id: str (Foreign Key -> Transaction.id)
    amount: Decimal
    reason: str
    status: RefundStatus  # 'pending' | 'processing' | 'success' | 'failed'
    error_message: str (Nullable)
    processed_by: int (Foreign Key -> User.id)
    processed_at: DateTime (Nullable)
    created_at: DateTime
    updated_at: DateTime
```

### 3.3 Transaction 模型擴充
```python
class Transaction(Base):
    # 原有欄位...

    # 新增欄位
    is_refunded: bool = False
    refunded_at: DateTime (Nullable)
    refund_amount: Decimal (Nullable)
```

---

## 四、API 規格

### 4.1 角色管理 API

#### 取得當前用戶角色
```
GET /api/teachers/me/role
Response: {
  "subscription_tier": "tutor",
  "trial_credits_remaining": 500,
  "subscription_credits_remaining": 8500,
  "subscription_credits_total": 10000
}
```

### 4.2 退款管理 API

#### 申請退款
```
POST /api/admin/refunds
Request: {
  "transaction_id": "txn_123456",
  "amount": 330,
  "reason": "用戶要求退款"
}
Response: {
  "refund_id": "ref_789",
  "status": "processing"
}
```

#### 查詢退款狀態
```
GET /api/admin/refunds/{refund_id}
Response: {
  "id": "ref_789",
  "status": "success",
  "processed_at": "2025-11-15T10:30:00Z"
}
```

### 4.3 帳款匯出 API

#### 匯出交易明細
```
GET /api/admin/transactions/export?start_date=2025-11-01&end_date=2025-11-30&format=xlsx
Response: File Download (transactions_20251130_143000.xlsx)
```

### 4.4 語系管理 API

#### 更新語系偏好
```
PUT /api/teachers/me/language
Request: {
  "language": "en"
}
Response: {
  "language": "en",
  "updated_at": "2025-11-17T09:00:00Z"
}
```

---

## 五、測試計畫

### 5.1 單元測試
- [ ] 角色權限驗證測試
- [ ] 點數計算邏輯測試
- [ ] 退款流程測試
- [ ] 語系切換測試
- [ ] 帳款匯出測試

### 5.2 整合測試
- [ ] TapPay 退款 API 整合測試
- [ ] 點數扣除與重置測試
- [ ] 多語言 Email 發送測試
- [ ] CSV/XLSX 匯出正確性測試

### 5.3 E2E 測試
- [ ] 用戶升級訂閱流程
- [ ] 管理者執行退款流程
- [ ] 語系切換完整流程
- [ ] 帳款明細匯出流程

### 5.4 效能測試
- [ ] 大量交易資料匯出（10,000 筆以上）
- [ ] 語系檔載入效能
- [ ] 點數計算查詢效能

---

## 六、部署計畫

### 6.1 部署步驟
1. **資料庫遷移**
   ```bash
   alembic revision --autogenerate -m "November 2025 features"
   alembic upgrade head
   ```

2. **環境變數更新**
   ```bash
   # 新增退款相關設定
   TAPPAY_REFUND_API_URL=https://sandbox.tappaysdk.com/tpc/refund/refund

   # 新增語系設定
   DEFAULT_LANGUAGE=zh
   SUPPORTED_LANGUAGES=zh,en
   ```

3. **靜態資源更新**
   - 語系檔部署到 CDN
   - 前端 build 並部署

4. **Cron Job 設定**
   ```bash
   # 每月1日 00:00 重置訂閱點數
   0 0 1 * * python backend/scripts/reset_subscription_credits.py
   ```

### 6.2 部署檢查清單
- [ ] 資料庫備份完成
- [ ] Staging 環境測試通過
- [ ] 環境變數正確設定
- [ ] Cron Job 正確設定
- [ ] 語系檔正確部署
- [ ] Email 模板正確更新
- [ ] 監控告警設定完成

---

## 七、文件更新

### 7.1 需要更新的文件
- [ ] `PRD.md` - 更新 Phase 1.5 內容
- [ ] `CLAUDE.md` - 新增 i18n 開發規範
- [ ] `README.md` - 更新功能清單
- [ ] `API_DOCS.md` - 新增 API 文件
- [ ] `DEPLOYMENT.md` - 更新部署流程

### 7.2 新增文件
- [ ] `i18n_GUIDE.md` - 多語言開發指南
- [ ] `ADMIN_MANUAL.md` - 管理者操作手冊
- [ ] `REFUND_POLICY.md` - 退款政策說明

---

## 八、風險評估

### 8.1 技術風險
| 風險項目 | 影響程度 | 緩解措施 |
|---------|---------|---------|
| TapPay 退款 API 整合失敗 | 高 | 完整的錯誤處理與重試機制 |
| 大量資料匯出超時 | 中 | 非同步處理 + 進度通知 |
| 語系檔缺失翻譯 | 低 | Fallback 機制 + 翻譯完整性檢查 |
| 點數計算邏輯錯誤 | 高 | 完整單元測試 + 人工驗證 |

### 8.2 業務風險
| 風險項目 | 影響程度 | 緩解措施 |
|---------|---------|---------|
| 退款流程濫用 | 中 | 退款原因記錄 + 異常偵測 |
| 點數累計規則誤解 | 中 | 清楚的 UI 說明 + FAQ |
| 多語言翻譯品質 | 低 | 專業翻譯審核 |

---

## 九、驗收標準

### 9.1 功能驗收
- [ ] 所有功能依照規格完成
- [ ] 所有 API 正常運作
- [ ] 所有單元測試通過
- [ ] 所有整合測試通過
- [ ] E2E 測試通過

### 9.2 品質驗收
- [ ] 程式碼通過 ESLint 檢查
- [ ] 程式碼通過 flake8 檢查
- [ ] 測試覆蓋率 > 80%
- [ ] 無重大 bug
- [ ] 效能符合標準

### 9.3 文件驗收
- [ ] API 文件完整
- [ ] 使用者手冊完整
- [ ] 管理者手冊完整
- [ ] 部署文件更新

---

## 十、每月自動續訂 Cron Job (2025-11-17 開發中)

### 10.1 功能描述
實作訂閱自動續訂的 Cron Job，每月 1 日凌晨 2:00 自動處理訂閱到期與續訂邏輯。

### 10.2 執行時間
- **觸發時間**: 每月 1 日 02:00 (台北時間)
- **Cron 表達式**: `0 2 1 * *`

### 10.3 Cron Job 邏輯

#### Phase 1: 標記過期訂閱
```
目標: 將所有到期的 active 訂閱標記為 expired
查詢條件:
  - status = 'active'
  - end_date < 當月 1 日
動作:
  - 更新 status = 'expired'
  - 記錄數量: marked_expired
```

#### Phase 2: 自動續訂處理
```
目標: 處理所有符合續訂條件的教師
查詢條件:
  - subscription_auto_renew = True
  - is_active = True
  - 有綁定信用卡 (card_key + card_token)

處理流程:
  1. 檢查 1: 防重複扣款
     - 查詢是否已有本月訂閱 (start_date >= 當月 1 日)
     - 如有 → 跳過 (skip_count++)

  2. 檢查 2: 防錯誤扣款 + 自動關閉 auto_renew
     - 查詢是否有上個月訂閱 (start_date = 上月 1 日, end_date = 上月最後一天)
     - 如無 → 關閉 auto_renew + 發通知 (auto_renew_disabled++)

  3. 執行續訂
     - 呼叫 TapPay API 扣款
     - 成功 → 建立新訂閱期 (auto_renewed++)
     - 失敗 → 記錄錯誤 (renewal_failed++)
```

### 10.4 資料模型

#### SubscriptionPeriod 欄位
```python
class SubscriptionPeriod(Base):
    id: int
    teacher_id: int
    plan_name: str  # 'Tutor Teachers' | 'School Teachers'
    amount_paid: int  # 231 (Tutor) | 660 (School)
    quota_total: int  # 10000 (Tutor) | 25000 (School)
    start_date: date  # 當月 1 日
    end_date: date  # 當月最後一天
    payment_method: str  # 'auto_renew'
    status: str  # 'active' | 'expired' | 'cancelled'
    created_at: datetime
```

#### Teacher 相關欄位
```python
class Teacher(Base):
    subscription_auto_renew: bool  # 是否自動續訂
    card_key: str  # TapPay card_key
    card_token: str  # TapPay card_token
    is_active: bool  # 帳號是否啟用
```

### 10.5 回應格式
```json
{
  "status": "completed",
  "date": "2025-12-01",
  "marked_expired": 15,
  "auto_renewed": 8,
  "renewal_failed": 2,
  "auto_renew_disabled": 1,
  "skipped": 3,
  "errors": [
    {
      "teacher_id": 123,
      "error": "TapPay API error: insufficient funds"
    }
  ]
}
```

### 10.6 測試策略 (TDD)

#### 測試檔案
`backend/tests/integration/api/test_monthly_renewal_cron.py`

#### 測試場景 (16 個)

**Phase 1: 標記過期訂閱 (3 tests)**
1. ✅ 標記上月到期的 active 訂閱
2. ✅ 標記多個月前到期的 active 訂閱
3. ✅ 不標記當月的 active 訂閱

**Phase 2: 自動續訂 (3 tests)**
4. ✅ 成功續訂（有上月訂閱 + auto_renew + 綁卡）
5. ✅ 續訂失敗（TapPay API 錯誤）
6. ✅ 續訂後正確更新 quota

**檢查 1: 防重複扣款 (2 tests)**
7. ✅ 跳過已有本月訂閱的教師
8. ✅ 不跳過沒有本月訂閱的教師

**檢查 2: 關閉 auto_renew (2 tests)**
9. ✅ 關閉無上月訂閱的 auto_renew
10. ✅ 不關閉有上月訂閱的 auto_renew

**跳過場景 (3 tests)**
11. ✅ 跳過 auto_renew = False 的教師
12. ✅ 跳過沒綁卡的教師
13. ✅ 跳過 is_active = False 的教師

**邊界案例 (3 tests)**
14. ✅ 處理沒有任何訂閱的教師
15. ✅ 處理 trial 訂閱用戶
16. ✅ 處理多個教師同時續訂

### 10.7 開發進度

#### ✅ 已完成 (2025-11-17)
- [x] TDD 測試套件撰寫 (16 tests) - **100% PASSING**
- [x] 測試資料 fixtures 設定
- [x] 時間 mocking (freezegun)
- [x] TapPay service mocking
- [x] Git branch: `prd_nov` 建立
- [x] 初始 commit 完成
- [x] 信用卡綁定流程 + 自動續訂確認對話框 (前端)
- [x] PRD 第十章：信用卡綁定與自動續訂規範完成
- [x] **Cron API endpoint 完整實作** (`POST /api/cron/monthly-renewal`)
- [x] Phase 1: 標記過期訂閱邏輯 ✅
- [x] Phase 2: 自動續訂核心邏輯 ✅
- [x] 檢查 1: 防重複扣款邏輯 ✅
- [x] 檢查 2: 關閉 auto_renew 邏輯 ✅
- [x] 測試驗證 (TDD GREEN) - 16/16 tests ✅

#### 🔄 進行中
- [ ] 無（開發已完成，等待部署）

#### ⏳ 待處理
- [ ] Cloud Scheduler 設定 (每月 1 日 02:00)
- [ ] Email 通知（auto_renew 關閉通知）
- [ ] 錯誤監控與告警 (Cloud Logging)
- [ ] Staging 環境測試
- [ ] Production 部署

### 10.8 驗收標準
- [x] 所有 16 個測試通過 ✅ (2025-11-17)
- [x] Phase 1 正確標記所有過期訂閱 ✅
- [x] Phase 2 正確處理續訂邏輯 ✅
- [x] 防重複扣款機制正常運作 ✅
- [x] 自動關閉 auto_renew 機制正常運作 ✅
- [x] 回應格式符合規格 ✅
- [x] 錯誤處理完善 ✅
- [x] Email 通知實作 (send_renewal_success) ✅
- [x] ✅ **已部署到 main** (2025-11-18)
- [ ] Cloud Scheduler 正確觸發 Cron Job (待部署)
- [ ] Production 環境成功執行第一次續訂 (待驗證)

---

## 十一、後續工作

### 11.1 12月規劃
- 統計圖表功能增強
- 批量操作優化
- 效能監控改善
- 使用者回饋收集

### 11.2 Phase 2 準備
- 學校管理功能設計
- 多教師協作機制
- 進階報表系統
- 家長功能規劃

---

## 附錄

### A. 翻譯對照表（常用詞彙）
| 中文 | 英文 |
|-----|------|
| 儀表板 | Dashboard |
| 班級 | Classroom |
| 學生 | Student |
| 課程 | Program/Course |
| 作業 | Assignment |
| 訂閱 | Subscription |
| 試用 | Trial |
| 點數 | Credits |
| 退款 | Refund |
| 帳款 | Transaction |

### B. 相關連結
- [TapPay Refund API 文件](https://docs.tappaysdk.com/refund/zh/front.html)
- [react-i18next 官方文件](https://react.i18next.com/)
- [OpenPyXL 文件](https://openpyxl.readthedocs.io/)

---

## 十、信用卡綁定與自動續訂流程規範

### 10.1 功能概述
定義前端信用卡綁定、解綁與自動續訂之間的互動邏輯，確保使用者體驗一致且符合商業邏輯。

### 10.2 核心規則

#### 規則 1: 綁卡時詢問自動續訂
**情境**: 使用者首次綁定信用卡或更換信用卡

**流程**:
```
1. 使用者輸入信用卡資訊
2. TapPay 驗證成功
3. ❓ 顯示確認對話框: "是否啟用自動續訂？"
   - 選項 A: "是，自動續訂" → subscription_auto_renew = True
   - 選項 B: "否，手動續訂" → subscription_auto_renew = False
4. 儲存綁卡資訊 (card_key, card_token) + auto_renew 設定
5. 前端更新顯示狀態
```

**UI 要點**:
- 清楚說明自動續訂的意義（每月 1 號自動扣款）
- 提供兩個選項，預設不勾選（使用者主動選擇）
- 說明可隨時在設定中變更

**API 呼叫**:
```typescript
POST /api/teachers/bind-card
{
  "card_key": "...",
  "card_token": "...",
  "auto_renew": true  // 根據使用者選擇
}
```

**後端驗證**:
```python
# routers/payment.py or routers/teachers.py
def bind_card(teacher: Teacher, card_key: str, card_token: str, auto_renew: bool):
    teacher.card_key = card_key
    teacher.card_token = card_token
    teacher.subscription_auto_renew = auto_renew
    db.commit()
```

---

#### 規則 2: 刪除綁卡時自動關閉續訂
**情境**: 使用者解除信用卡綁定

**業務邏輯**:
- 沒有綁卡 → 無法自動扣款 → auto_renew 必須關閉
- 避免使用者誤以為會自動續訂，但實際上無法扣款

**流程**:
```
1. 使用者點擊「刪除信用卡」
2. ❓ 顯示確認對話框:
   "刪除信用卡後，自動續訂將會關閉。確定要刪除嗎？"
3. 使用者確認
4. 後端執行:
   - card_key = NULL
   - card_token = NULL
   - subscription_auto_renew = False  ← 強制關閉
5. 前端即時更新:
   - 隱藏信用卡資訊顯示
   - 自動續訂開關變為 OFF（且 disabled）
   - 顯示提示: "請先綁定信用卡才能啟用自動續訂"
```

**API 呼叫**:
```typescript
DELETE /api/teachers/card
// 或
POST /api/teachers/unbind-card
```

**後端實作**:
```python
@router.delete("/card")
def unbind_card(
    teacher: Teacher = Depends(get_current_teacher),
    db: Session = Depends(get_db)
):
    # 刪除綁卡資訊
    teacher.card_key = None
    teacher.card_token = None

    # 🔴 強制關閉自動續訂
    teacher.subscription_auto_renew = False

    db.commit()

    return {
        "message": "信用卡已解綁，自動續訂已關閉",
        "card_bound": False,
        "auto_renew": False
    }
```

**前端連動**:
```typescript
// 刪除綁卡後，UI 必須即時更新
const handleUnbindCard = async () => {
  await api.unbindCard();

  // 更新 UI 狀態
  setCardBound(false);
  setAutoRenew(false);
  setAutoRenewDisabled(true);  // 禁用自動續訂開關

  // 顯示提示
  showToast("信用卡已解綁，自動續訂已關閉");
}
```

---

#### 規則 3: 只取消續訂，保留綁卡
**情境**: 使用者只想關閉自動續訂，但保留信用卡資訊

**業務邏輯**:
- 使用者可能希望手動續訂，而非自動扣款
- 保留綁卡資訊方便未來使用

**流程**:
```
1. 使用者在「訂閱設定」頁面
2. 關閉「自動續訂」開關
3. ❓ 顯示確認對話框:
   "關閉自動續訂後，將不會在每月 1 號自動扣款。
    信用卡資訊會保留，您仍可手動續訂。確定要關閉嗎？"
4. 使用者確認
5. 後端執行:
   - subscription_auto_renew = False
   - card_key 保持不變
   - card_token 保持不變
6. 前端更新:
   - 自動續訂開關 OFF
   - 信用卡資訊仍然顯示
   - 顯示提示: "已關閉自動續訂，您可隨時手動續訂"
```

**API 呼叫**:
```typescript
PATCH /api/teachers/subscription-settings
{
  "auto_renew": false
}
```

**後端實作**:
```python
@router.patch("/subscription-settings")
def update_subscription_settings(
    settings: SubscriptionSettings,
    teacher: Teacher = Depends(get_current_teacher),
    db: Session = Depends(get_db)
):
    # 只更新 auto_renew，不動 card_key/card_token
    teacher.subscription_auto_renew = settings.auto_renew
    db.commit()

    return {
        "message": "訂閱設定已更新",
        "auto_renew": teacher.subscription_auto_renew,
        "card_bound": bool(teacher.card_key)  # 綁卡狀態不變
    }
```

---

### 10.3 狀態矩陣

| 綁卡狀態 | auto_renew | 允許？ | 說明 |
|---------|-----------|-------|------|
| ❌ 未綁卡 | False | ✅ 允許 | 初始狀態 / 解綁後狀態 |
| ❌ 未綁卡 | True  | ❌ **禁止** | 不合理狀態，後端應拒絕 |
| ✅ 已綁卡 | False | ✅ 允許 | 手動續訂模式 |
| ✅ 已綁卡 | True  | ✅ 允許 | 自動續訂模式 |

**後端驗證規則**:
```python
# 在任何更新 auto_renew 的 API 中
def validate_auto_renew(teacher: Teacher, auto_renew: bool):
    if auto_renew and not teacher.card_key:
        raise HTTPException(
            status_code=400,
            detail="無法啟用自動續訂：尚未綁定信用卡"
        )
```

---

### 10.4 前端 UI 規範

#### UI 元素互動邏輯

**信用卡設定頁面**:
```tsx
<Card title="信用卡管理">
  {cardBound ? (
    <>
      <CardDisplay lastFour={cardInfo.lastFour} />
      <Button onClick={handleUnbindCard} variant="danger">
        刪除信用卡
      </Button>
    </>
  ) : (
    <Button onClick={handleBindCard} variant="primary">
      綁定信用卡
    </Button>
  )}
</Card>

<Card title="自動續訂設定">
  <Switch
    checked={autoRenew}
    disabled={!cardBound}  // ← 沒綁卡時禁用
    onChange={handleAutoRenewChange}
    label="每月自動續訂"
  />

  {!cardBound && (
    <HelpText variant="warning">
      請先綁定信用卡才能啟用自動續訂
    </HelpText>
  )}
</Card>
```

**綁卡對話框**:
```tsx
<Modal title="綁定信用卡" onClose={handleClose}>
  <TapPayCardForm onSuccess={handleCardSuccess} />

  <Divider />

  <Checkbox
    checked={enableAutoRenew}
    onChange={setEnableAutoRenew}
    label="啟用自動續訂"
  />
  <HelpText>
    啟用後，系統將在每月 1 號自動扣款並續訂。
    您可隨時在設定中關閉此功能。
  </HelpText>

  <ButtonGroup>
    <Button onClick={handleClose}>取消</Button>
    <Button onClick={handleConfirm} variant="primary">
      確認綁定
    </Button>
  </ButtonGroup>
</Modal>
```

---

### 10.5 測試案例

#### Test Case 1: 綁卡時選擇啟用自動續訂
```
Given: 使用者未綁卡，auto_renew = False
When: 使用者綁定信用卡，選擇「啟用自動續訂」
Then:
  - card_key 和 card_token 儲存成功
  - auto_renew = True
  - 前端顯示綁卡資訊
  - 自動續訂開關為 ON
```

#### Test Case 2: 綁卡時選擇不啟用自動續訂
```
Given: 使用者未綁卡，auto_renew = False
When: 使用者綁定信用卡，選擇「不啟用自動續訂」
Then:
  - card_key 和 card_token 儲存成功
  - auto_renew = False
  - 前端顯示綁卡資訊
  - 自動續訂開關為 OFF（但可手動開啟）
```

#### Test Case 3: 刪除綁卡，自動關閉續訂
```
Given: 使用者已綁卡，auto_renew = True
When: 使用者刪除信用卡綁定
Then:
  - card_key = NULL
  - card_token = NULL
  - auto_renew = False（強制關閉）
  - 前端隱藏綁卡資訊
  - 自動續訂開關為 OFF 且 disabled
```

#### Test Case 4: 只關閉自動續訂
```
Given: 使用者已綁卡，auto_renew = True
When: 使用者關閉自動續訂開關
Then:
  - card_key 保持不變
  - card_token 保持不變
  - auto_renew = False
  - 前端仍顯示綁卡資訊
  - 自動續訂開關為 OFF（可再次開啟）
```

#### Test Case 5: 未綁卡嘗試啟用自動續訂（後端防禦）
```
Given: 使用者未綁卡，card_key = NULL
When: 前端發送請求設定 auto_renew = True
Then:
  - 後端回傳 400 錯誤
  - 錯誤訊息: "無法啟用自動續訂：尚未綁定信用卡"
  - auto_renew 保持 False
```

---

### 10.6 API 規格

#### API 1: 綁定信用卡
```
POST /api/teachers/bind-card

Request:
{
  "card_key": "string",
  "card_token": "string",
  "auto_renew": boolean
}

Response:
{
  "message": "信用卡綁定成功",
  "card_bound": true,
  "auto_renew": true,
  "card_last_four": "1234"
}
```

#### API 2: 解除綁卡
```
DELETE /api/teachers/card

Response:
{
  "message": "信用卡已解綁，自動續訂已關閉",
  "card_bound": false,
  "auto_renew": false
}
```

#### API 3: 更新自動續訂設定
```
PATCH /api/teachers/subscription-settings

Request:
{
  "auto_renew": boolean
}

Response:
{
  "message": "訂閱設定已更新",
  "auto_renew": false,
  "card_bound": true
}

Error (400):
{
  "detail": "無法啟用自動續訂：尚未綁定信用卡"
}
```

---

### 10.7 驗收標準

#### 前端驗收
- [x] 綁卡時顯示「是否啟用自動續訂」選項 ✅ (ce701d5)
- [x] 綁卡後，UI 正確顯示綁卡狀態與自動續訂狀態 ✅
- [x] 刪除綁卡時顯示警告訊息（含自動關閉續訂說明） ✅
- [x] 刪除綁卡後，自動續訂開關自動關閉且禁用 ✅
- [x] 未綁卡時，自動續訂開關為禁用狀態 ✅
- [x] 關閉自動續訂時顯示確認對話框 ✅
- [x] 所有操作成功後顯示適當的 toast 訊息 ✅

#### 後端驗收
- [x] `POST /api/payment/update-card` 正確儲存 card + auto_renew ✅ (ce701d5)
- [x] `DELETE /api/payment/saved-card` 強制設定 auto_renew = False ✅
- [x] 後端驗證：未綁卡時拒絕設定 auto_renew = True ✅
- [x] 所有 API 正確回傳操作後的狀態 ✅
- [x] 資料庫約束：不允許 card_key=NULL 且 auto_renew=True ✅

#### 整合測試
- [x] 完整流程：綁卡 → 啟用續訂 → 關閉續訂 → 刪除綁卡 ✅ (已實作)
- [x] 邊界案例：未綁卡嘗試啟用續訂（應被拒絕） ✅
- [x] 邊界案例：刪除綁卡時，續訂自動關閉 ✅
- [x] UI 與後端狀態完全同步 ✅

---

### 10.8 實作優先順序

**Phase 1 (Must Have)**:
1. ✅ 後端 API: `DELETE /api/payment/saved-card` 強制關閉 auto_renew
2. ✅ 後端驗證: 拒絕未綁卡時設定 auto_renew = True
3. ✅ 前端: 刪除綁卡時自動關閉續訂 UI

**Phase 2 (Should Have)**:
4. ✅ 綁卡時詢問自動續訂對話框 (ce701d5)
5. ✅ 未綁卡時禁用自動續訂開關
6. ✅ 所有確認對話框與警告訊息

**Phase 3 (Nice to Have)**:
7. ⏳ 完整的 E2E 測試套件 (已有基礎測試)
8. ✅ 使用者引導（tooltips, help text）

---

**文件版本**: 1.1
**建立日期**: 2025-11-17
**最後更新**: 2025-11-17 (新增第十章：信用卡綁定規範)
**負責人**: 產品團隊
**狀態**: ✅ Final 版本
