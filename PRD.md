...
**Step 1：選擇內容**
- 顯示格式：Tree View 樹狀結構
  - Program → Lesson → Content 三層展開
  - 每個 Content 顯示類型標籤（朗讀評測）
  - 支援多選（checkbox）
  - 顯示預估完成時間
- 搜尋功能：
  - 按標題搜尋
...
# 查詢可用 Content
GET /api/contents?classroom_id=456
```
...
**API 擴充**：
- 教師查詢作業：`GET /api/assignments/teacher?classroom_id=456`
- 學生查詢作業：`GET /api/assignments/student`
- 提交作業：`POST /api/assignments/{id}/submit`

**介面開發**：
- 教師端：班級詳情頁新增「作業列表」tab
- 學生端：作業列表頁面with分類檢視
...
{
  "audio_url": "gs://bucket/recording.mp3"
}
# 返回 AI 評分結果
```
...
**資料庫**：
- 使用現有 StudentAssignment 模型
- 使用現有 AssignmentSubmission 模型
- 現有結構已足夠支援 Phase 1-2

**前端路由規劃**：
- `/classroom/:id/assignments` - 作業列表
- `/classroom/:id/assignments/new` - 新增作業
- `/assignments/:id/grade` - 批改介面
- `/student/assignments` - 學生作業列表
...
| 週次 | 開發內容 | 交付成果 |
|-----|---------|----------|
| Week 1 | Phase 1 開發 | 基礎指派功能上線 |
| Week 2 | Phase 2 開發 | 作業列表完成 |
| Week 3-4 | Phase 3 開發 | AI 批改整合 |
| Week 5 | Phase 4 開發 | 人工批改上線 |
| Week 6-7 | Phase 5 開發 | 進階功能完善 |
