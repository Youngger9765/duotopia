...
describe("CopyProgramDialog", () => {
  ...
  it("should display program details", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Basic English Course")).toBeInTheDocument();
      expect(
        screen.getByText("Foundation course for beginners"),
      ).toBeInTheDocument();
  ...
  it("should copy selected programs on submit", async () => {
    vi.mocked(apiClient.copyProgramToClassroom).mockResolvedValue({
      success: true,
    });
    ...
    await waitFor(() => {
      expect(apiClient.copyProgramToClassroom).toHaveBeenCalledWith(
        classroomId,
        [1, 2],
      );
  ...
  it("should handle copy error", async () => {
    vi.mocked(apiClient.copyProgramToClassroom).mockRejectedValue(
  ...
  it("should filter programs by search term", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("Advanced Speaking")).toBeInTheDocument();
      expect(
        screen.queryByText("Basic English Course"),
  ...
  it("should show empty state when no programs available", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("請先到「所有課程」頁面建立課程"),
      ).toBeInTheDocument();
    });
  });
