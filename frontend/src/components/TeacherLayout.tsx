...
export default function TeacherLayout({ children }: TeacherLayoutProps) {
  ...
  const SidebarContent = ({ onNavigate }: { onNavigate?: () => void }) => (
    <>
      ...
      <div className="p-4 border-b flex items-center justify-between">
        {!sidebarCollapsed && (
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Duotopia</h1>
              ...
              <div>
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{teacherProfile.name}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{teacherProfile.email}</p>
                  </div>
                </div>
              </div>
            )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <div className="flex">
        {/* Desktop Sidebar */}
        <div className={`hidden md:flex bg-white dark:bg-gray-800 shadow-lg transition-all duration-300 ${sidebarCollapsed ? 'w-16' : 'w-64'} flex-col h-screen sticky top-0`}>
          <SidebarContent />
        </div>
