...
import { PERMISSION_TEMPLATES, PermissionTemplateName, TeacherPermissions } from '@/lib/permissions';
...
export function PermissionTemplateSelector({
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="text-sm font-medium mb-2 block">Permission Template</label>
      ...
      {showPreview && selectedTemplate && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>{PERMISSION_TEMPLATES[selectedTemplate].description}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              <h4 className="text-sm font-semibold">Permissions:</h4>
              <ul className="space-y-1 text-sm">
                {Object.entries(PERMISSION_TEMPLATES[selectedTemplate].permissions).map(
                  ([key, value]) => {
                    if (key === 'allowed_actions') {
                      const actions = value as string[];
                      return (
                        <li key={key} className="flex items-start gap-2">
                          <span className="text-gray-600 min-w-[180px]">Allowed Actions:</span>
                    ...
                    if (key === 'max_classrooms') {
                      return (
                        <li key={key} className="flex items-start gap-2">
                          <span className="text-gray-600 min-w-[180px]">Max Classrooms:</span>
