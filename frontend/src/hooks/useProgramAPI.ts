...
export function useProgramAPI(options: ProgramAPIOptions) {
  ...
  return {
    ...
    updateProgram: async (id: number, data: { name?: string; description?: string }) => {
    ...
    createLesson: async (programId: number, data: { name: string; description?: string; order_index?: number }) => {
      const response = await fetch(buildURL(`/api/programs/${programId}/lessons`), {
        method: 'POST',
        headers,
        body: JSON.stringify(data),
      });
    ...
    updateLesson: async (lessonId: number, data: { name?: string; description?: string }) => {
      const response = await fetch(buildURL(`/api/programs/lessons/${lessonId}`), {
        method: 'PUT',
        headers,
        body: JSON.stringify(data),
      });
    ...
    createContent: async (lessonId: number, data: { type: string; title: string; order_index?: number }) => {
      const response = await fetch(buildURL(`/api/programs/lessons/${lessonId}/contents`), {
        method: 'POST',
        headers,
        body: JSON.stringify(data),
      });
    ...
    deleteContent: async (contentId: number) => {
      const response = await fetch(buildURL(`/api/programs/contents/${contentId}`), {
        method: 'DELETE',
        headers,
      });
