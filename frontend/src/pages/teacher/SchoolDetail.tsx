...
export default function SchoolDetail() {
  ...
  const [availableTeachersLoading, setAvailableTeachersLoading] =
    useState(false);
  ...
  const fetchAvailableTeachers = async () => {
    if (!school?.organization_id) return;

    try {
      setAvailableTeachersLoading(true);
      const response = await fetch(
        `${API_URL}/api/organizations/${school.organization_id}/teachers`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      if (response.ok) {
        const data = await response.json();
        // Filter out teachers already in this school
        const existingTeacherIds = new Set(teachers.map((t) => t.id));
        const available = data.filter(
          (t: Teacher) => !existingTeacherIds.has(t.id),
        );
        setAvailableTeachers(available);
      } else {
        toast.error("無法載入可用教師列表");
      }
    } catch (error) {
      console.error("Failed to fetch available teachers:", error);
      toast.error("載入教師列表時發生錯誤");
    } finally {
  ...
  const handleAddTeacher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        handleCloseModal();
        fetchTeachers();
        toast.success("教師已成功加入學校");
      } else {
        const error = await response.json();
        toast.error(`新增失敗: ${error.detail}`);
      }
    } catch (error) {
      console.error("Add teacher failed:", error);
      toast.error("新增教師時發生錯誤");
    }
  };
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-8 max-w-md w-full">
                ...
                <form onSubmit={handleAddTeacher} className="space-y-4">
                  <div>
                    <label className="block mb-2">選擇教師 *</label>
