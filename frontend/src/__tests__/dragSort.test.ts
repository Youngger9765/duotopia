...
describe("拖拽排序功能", () => {
  ...
  describe("批量 API 更新", () => {
    it("應該批量更新所有內容的 order_index", async () => {
      ...
      expect(mockUpdateContent).toHaveBeenNthCalledWith(1, 1, {
        order_index: 3,
      });
      expect(mockUpdateContent).toHaveBeenNthCalledWith(2, 2, {
        order_index: 1,
      });
      expect(mockUpdateContent).toHaveBeenNthCalledWith(3, 3, {
        order_index: 2,
      });
    });
