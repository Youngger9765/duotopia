...
class TestStudentManagementWorkflow:
    ...
    def test_full_student_management_workflow(self):
        ...
        response = self.client.get(
            "/api/teachers/classrooms", headers=self.teacher_headers
        )
        ...
        ), f"建立學生失敗: {response.status_code} - {response.text}"
        ...
        # 4. 確認學生列表
        response = self.client.get(
            "/api/teachers/students", headers=self.teacher_headers
        )
        ...
        assert (
            response.status_code == 200
        ), f"分配班級失敗: {response.status_code} - {response.text}"
        ...
        response = self.client.delete(
            f"/api/teachers/students/{student_id}", headers=self.teacher_headers
        )

        assert (
            response.status_code == 200
        ), f"刪除學生失敗: {response.status_code} - {response.text}"
    ...
    def test_student_creation_validation(self):
        ...
        response = self.client.post(
            "/api/teachers/students", json=invalid_data, headers=self.teacher_headers
        )
    ...
    def test_student_deletion_permissions(self):
        ...
        try:
            ...
            # 測試刪除不存在的學生
            response = self.client.delete(
                "/api/teachers/students/99999", headers=self.teacher_headers
            )
    ...
    def test_student_list_filtering(self):
        ...
        # 測試搜尋功能（如果 API 支援）
        response = self.client.get(
            "/api/teachers/students?search=test", headers=self.teacher_headers
        )
    ...
    def test_student_data_consistency(self):
        ...
        response = self.client.post(
            "/api/teachers/students", json=original_data, headers=self.teacher_headers
        )
