...
def test_seed_data(headers):
    ...
    for program in programs:
        detail_resp = requests.get(
            f"{BASE_URL}/api/teachers/programs/{program['id']}", headers=headers
        )
        ...
        if program["id"] <= 4:  # 檢查前 4 個課程（seed data）
            assert (
                len(lessons) >= 3
            ), f"課程 {program['name']} 應有 3 個單元，實際: {len(lessons)}"
...
def test_program_crud(headers):
    ...
    create_resp = requests.post(
        f"{BASE_URL}/api/teachers/programs", headers=headers, json=create_data
    )
    ...
    delete_resp = requests.delete(
        f"{BASE_URL}/api/teachers/programs/{program_id}", headers=headers
    )
...
def test_frontend_health():
    ...
    try:
        ...
        assert (
            "Duotopia" in response.text or "duotopia" in response.text.lower()
        ), "前端內容異常"
        print("✅ 前端運行正常")
        return True
