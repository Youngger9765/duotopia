...
def test_complete_flow():
    ...
    # 3. 查詢學生列表
    print("\n3️⃣ 查詢學生列表...")
    response = requests.get(f"{BASE_URL}/classrooms/{classroom_id}/students", headers=teacher_headers)
    ...
    # 4. 查詢課程內容
    print("\n4️⃣ 查詢課程內容...")
    response = requests.get(f"{BASE_URL}/contents?classroom_id={classroom_id}", headers=teacher_headers)
    ...
    response = requests.post(f"{BASE_URL}/assignments/create", 
    ...
    # 6. 查詢教師的作業列表
    print("\n6️⃣ 查詢教師的作業列表...")
    response = requests.get(f"{BASE_URL}/assignments/teacher?classroom_id={classroom_id}", 
    ...
    if students:
        student = students[0]
        response = requests.post(f"{BASE_URL}/auth/student/login", 
            json={"email": student["email"], "password": student["birthdate"].replace("-", "")})
