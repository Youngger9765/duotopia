...
class TestAssignmentsIntegration:
    ...
    STUDENT_CREDS = [
        {"name": "王小明", "email": "xiaoming.wang@duotopia.local", "password": "mynewpassword123"},
    ...
    def test_student_view_assignments(self):
        ...
        response = requests.get(
            f"{self.BASE_URL}/assignments/student",
            headers=headers
        )
    ...
    def test_student_view_assignment_detail(self):
        ...
        response = requests.get(
            f"{self.BASE_URL}/assignments/{self.assignment_id}/detail",
            headers=headers
        )
