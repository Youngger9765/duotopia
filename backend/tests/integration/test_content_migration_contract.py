...
class TestContentMigrationContract:
    ...
    def setup_method(self):
        ...
        self.audio_file = (
            "test_audio.webm",
            io.BytesIO(self.audio_content),
            "audio/webm",
        )

    def create_test_assignment_with_3_items(
        self, db: Session, teacher: User, student: User
    ) -> StudentAssignment:
        """Helper: Create assignment with 3 content items"""
        # Create content with 3 items
        content = Content(
            title="Test Pronunciation", type="pronunciation", teacher_id=teacher.id
        )
        db.add(content)
        db.flush()
    ...
    @pytest.mark.asyncio
    async def test_student_recording_upload_contract(
        self,
        ...
        """
        # Given: 學生有一個作業，包含3題
        assignment = self.create_test_assignment_with_3_items(
        ...
        try:
            ...
            item_progress = (
                ...
                .filter_by(
                    content_item_id=test_db.query(ContentItem)
                    .filter_by(content_id=assignment.content_id, order_index=1)
                    .first()
                    .id
                )
                .first()
            ...
            ), f"Status should be COMPLETED, got {item_progress.status}"

            # 2. 其他題目不受影響
            item_0_content = (
                test_db.query(ContentItem)
                .filter_by(content_id=assignment.content_id, order_index=0)
                .first()
            )
            item_2_content = (
                test_db.query(ContentItem)
                .filter_by(content_id=assignment.content_id, order_index=2)
                .first()
            )
            ...
            if item_0_progress:
                assert (
                    item_0_progress.recording_url is None
                ), "Item 0 should not have recording"
            if item_2_progress:
                assert (
                    item_2_progress.recording_url is None
                ), "Item 2 should not have recording"

            # 3. 摘要統計自動更新
            summary = (
                test_db.query(StudentContentProgress)
                .filter_by(student_assignment_id=assignment.id)
                .first()
            )
            assert (
                summary.completed_items == 1
            ), f"Completed items should be 1, got {summary.completed_items}"
            assert (
                abs(summary.completion_rate - 33.33) < 0.1
            ), f"Completion rate should be ~33.33, got {summary.completion_rate}"
    ...
    @pytest.mark.asyncio
    async def test_student_activities_view_contract(
        self,
        test_client: TestClient,
        test_db: Session,
        test_teacher: User,
        ...
        content_items = (
            test_db.query(ContentItem)
            .filter_by(content_id=assignment.content_id)
        ...
        assert (
            len(content1["items"]) == 3
        ), f"Content should have 3 items, got {len(content1['items'])}"

        # 2. 進度狀態正確
        item1 = content1["items"][0]
        assert (
            item1["status"] == "COMPLETED"
        ), f"Item 1 should be COMPLETED, got {item1['status']}"
        assert item1["recording_url"] is not None, "Item 1 should have recording URL"

        item2 = content1["items"][1]
        assert (
            item2["status"] == "IN_PROGRESS"
        ), f"Item 2 should be IN_PROGRESS, got {item2['status']}"

        # 3. AI 評分正確對應
        assert (
            float(item1["accuracy_score"]) == 85.5
        ), f"Accuracy score should be 85.5, got {item1['accuracy_score']}"
        assert (
            float(item1["fluency_score"]) == 78.9
        ), f"Fluency score should be 78.9, got {item1['fluency_score']}"

    @pytest.mark.asyncio
    async def test_ai_assessment_storage_contract(
        self,
        test_client: TestClient,
        test_db: Session,
        test_teacher: User,
        ...
        login_response = test_client.post(
            "/api/auth/login",
            json={"email": test_teacher.email, "password": "teacher123"},
        )
        token = login_response.json()["access_token"]
        headers = {"Authorization": f"Bearer {token}"}
        ...
        ), f"AI assessment storage failed: {response.text}"

        # 1. 評分正確儲存
        updated_progress = (
            test_db.query(StudentItemProgress).filter_by(id=item_progress.id).first()
        )
    ...
    @pytest.mark.asyncio
    async def test_teacher_progress_view_contract(
        self,
        test_client: TestClient,
        test_db: Session,
        test_teacher: User,
        ...
        assert (
            response.status_code == 200
        ), f"Teacher progress view failed: {response.text}"
    ...
    @pytest.mark.asyncio
    async def test_teacher_content_creation_contract(
        self, test_client: TestClient, test_db: Session, test_teacher: User
    ):
        """
        ...
        for i, item in enumerate(items):
            assert (
                item.order_index == i
            ), f"Item {i} should have order_index {i}, got {item.order_index}"
