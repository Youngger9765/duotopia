...
def teacher_with_comprehensive_data(db):
    ...
    classroom_students = [
        ClassroomStudent(classroom_id=classrooms[0].id, student_id=students[0].id, is_active=True),
        ClassroomStudent(classroom_id=classrooms[0].id, student_id=students[1].id, is_active=True),
        ClassroomStudent(classroom_id=classrooms[1].id, student_id=students[2].id, is_active=True),
        ClassroomStudent(classroom_id=classrooms[1].id, student_id=students[3].id, is_active=True),
        ClassroomStudent(classroom_id=classrooms[2].id, student_id=students[4].id, is_active=True),
    ]
...
class TestTeacherProfile:
    ...
    def test_get_profile_success(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers):
    ...
    def test_get_profile_non_teacher_token(self, client, teacher_with_comprehensive_data):
...
class TestTeacherDashboard:
    ...
    def test_dashboard_success(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers):
        """Test successful dashboard retrieval"""
        response = client.get("/api/teachers/dashboard", headers=comprehensive_auth_headers)
...
class TestDataIntegrityAndEdgeCases:
    ...
    def test_inactive_students_not_counted(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers, db):
    ...
    def test_empty_classrooms_handled_correctly(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers, db):
        ...
        empty_classroom_data = next(
            (c for c in data["classrooms"] if c["name"] == "Empty Classroom"),
            None
        )
        assert empty_classroom_data is not None
        assert empty_classroom_data["student_count"] == 0
    
    def test_null_descriptions_handled(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers, db):
        ...
        null_desc_data = next(
            (c for c in data["classrooms"] if c["name"] == "Null Desc Classroom"),
            None
        )
        assert null_desc_data is not None
        assert null_desc_data["description"] is None
    
    def test_classroom_student_relationship_integrity(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers, db):
        ...
        classroom = db.query(Classroom).filter(
            Classroom.teacher_id == teacher_with_comprehensive_data.id
        ).all()[-1]  # Last classroom
        ...
        assert data["student_count"] == 6  # Counts ClassroomStudent relationships, not unique students
    
    def test_recent_students_limit(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers, db):
...
class TestErrorRecovery:
    ...
    def test_concurrent_modifications(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers, db):
        """Test behavior when data is modified during request"""
        # Get initial data
        response1 = client.get("/api/teachers/dashboard", headers=comprehensive_auth_headers)
        ...
        # Get data again
        response2 = client.get("/api/teachers/dashboard", headers=comprehensive_auth_headers)
        assert response2.status_code == 200
        
        # Should reflect the changes
        data = response2.json()
        modified_classroom = next(
            (c for c in data["classrooms"] if c["name"] == "Modified Name"),
            None
        )
