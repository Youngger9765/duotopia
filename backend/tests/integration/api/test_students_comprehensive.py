...
class TestStudentLogin:
    ...
    def test_valid_student_login_email_format(
        self, db_session: Session, test_student: Student
    ):
    ...
    def test_valid_student_login_student_id_format(
        self, db_session: Session, test_student: Student
    ):
        ...
        response = client.post(
            "/api/students/login", json={"identifier": "STU001", "password": "20000101"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["student"]["id"] == test_student.id
    ...
    def test_empty_credentials(self):
        ...
        response = client.post(
            "/api/students/login", json={"identifier": "", "password": ""}
        )
        assert response.status_code == 401
...
class TestStudentProfile:
    ...
    def test_get_student_profile_success(
        self, test_student: Student, student_token: str
    ):
    ...
    def test_get_profile_invalid_token(self):
        ...
        response = client.get(
            "/api/students/me", headers={"Authorization": "Bearer invalid_token"}
        )
        assert response.status_code == 401
    ...
    def test_get_profile_teacher_token(self, teacher_token: str):
        ...
        response = client.get(
            "/api/students/me", headers={"Authorization": f"Bearer {teacher_token}"}
        )
...
class TestStudentDashboard:
    ...
    def test_get_dashboard_success(
        ...
        classroom_student = ClassroomStudent(
            classroom_id=test_classroom.id, student_id=test_student.id, is_active=True
        )
    ...
    def test_get_dashboard_invalid_token(self):
        ...
        response = client.get(
            "/api/students/dashboard", headers={"Authorization": "Bearer invalid_token"}
        )
        assert response.status_code == 401
...
class TestStudentAuthentication:
    ...
    def test_nonexistent_student_token(self, db_session: Session):
        ...
        response = client.get(
            "/api/students/me", headers={"Authorization": f"Bearer {fake_token}"}
        )
    ...
    def test_malformed_token_payload(self):
        ...
        response = client.get(
            "/api/students/me", headers={"Authorization": f"Bearer {malformed_token}"}
        )
        assert response.status_code == 403
