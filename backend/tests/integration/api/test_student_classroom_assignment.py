...
def test_student_classroom_assignment():
    ...
    response = requests.put(
        f"{BASE_URL}/teachers/students/{student_id}", json=update_data, headers=headers
    )
    ...
    if response.status_code == 200:
        ...
        if test_student and test_student["classroom_id"] == classroom_id:
            ...
            print(
                f"❌ 學生分配失敗 - classroom_id: {test_student['classroom_id'] if test_student else 'None'}"
            )
            assert False, "Test failed"
    ...
    response = requests.post(
        f"{BASE_URL}/teachers/classrooms", json=classroom2_data, headers=headers
    )
