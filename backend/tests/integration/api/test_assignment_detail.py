...
def test_student_get_assignment_detail():
    ...
    # 1. 學生登入
    print("1. 學生登入...")
    response = requests.post(f"{BASE_URL}/auth/student/login", json={
...
def test_teacher_view_submissions():
    ...
    # 1. 教師登入
    print("1. 教師登入...")
    response = requests.post(f"{BASE_URL}/auth/teacher/login", json={
    ...
    print("\n2. 獲取班級作業...")
    response = requests.get(
        f"{BASE_URL}/assignments/teacher?classroom_id=1",
