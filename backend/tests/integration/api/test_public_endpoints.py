...
def public_teacher_with_data(db, public_teacher):
    ...
    classroom_assignments = [
        ClassroomStudent(
            classroom_id=classroom1.id, student_id=students[0].id, is_active=True
        ),
        ClassroomStudent(
            classroom_id=classroom1.id, student_id=students[1].id, is_active=True
        ),
        ClassroomStudent(
            classroom_id=classroom1.id, student_id=students[2].id, is_active=True
        ),
        ClassroomStudent(
            classroom_id=classroom2.id, student_id=students[3].id, is_active=True
        ),
        ClassroomStudent(
            classroom_id=classroom2.id, student_id=students[4].id, is_active=True
        ),
    ]
...
class TestGetClassroomStudents:
    ...
    def test_get_students_empty_classroom(self, client, public_teacher_with_data):
        ...
        classroom = public_teacher_with_data["classrooms"][
            2
        ]  # Inactive Class (no students)
...
class TestDataIntegrityPublic:
    ...
    def test_student_count_accuracy(self, client, public_teacher_with_data, db):
        ...
        for classroom_data in classrooms:
            actual_count = (
                ...
                .filter(ClassroomStudent.classroom_id == classroom_data["id"])
                .count()
            )
    ...
    def test_students_from_correct_classroom(self, client, public_teacher_with_data):
        ...
        for student in public_teacher_with_data["students"][
            :3
        ]:  # First 3 students are in classroom 1
...
class TestErrorHandlingPublic:
    ...
    def test_sql_injection_protection(self, client, public_teacher):
        ...
        for email in malicious_emails:
            ...
            response = client.post(
                "/api/public/validate-teacher", json={"email": email}
            )
