...
def create_org_via_api(
    client: TestClient, headers: dict, name: str, teacher_limit: int = None
) -> dict:
...
def add_teacher_via_api(
    client: TestClient, headers: dict, org_id: str, teacher_id: int, role: str
):
    """Helper to add teacher to organization via API."""
    payload = {"teacher_id": teacher_id, "role": role}
    response = client.post(
        f"/api/organizations/{org_id}/teachers", json=payload, headers=headers
    )
    return response
...
class TestAddTeacherToOrganization:
    ...
    def test_add_teacher_success_org_admin(
        self, test_client: TestClient, auth_headers: dict, shared_test_session: Session
    ):
        ...
        response = add_teacher_via_api(
            test_client, auth_headers, org_id, teacher2_id, "org_admin"
        )
    ...
    def test_add_teacher_org_owner_limit(
        ...
        response = add_teacher_via_api(
            test_client, auth_headers, org_id, teacher2_id, "org_owner"
        )
    ...
    def test_add_teacher_teacher_limit_enforcement(
        ...
        org = create_org_via_api(
            test_client, auth_headers, "Test Org - Limit", teacher_limit=2
        )
        org_id = org["id"]
        ...
        response1 = add_teacher_via_api(
            test_client, auth_headers, org_id, teachers[0].id, "org_admin"
        )
        assert response1.status_code in [200, 201]

        # Add second teacher (count = 2) - should succeed
        response2 = add_teacher_via_api(
            test_client, auth_headers, org_id, teachers[1].id, "org_admin"
        )
        assert response2.status_code in [200, 201]

        # Add third teacher (count = 3, exceeds limit) - should fail
        response3 = add_teacher_via_api(
            test_client, auth_headers, org_id, teachers[2].id, "org_admin"
        )
    ...
    def test_add_teacher_to_nonexistent_org(
        ...
        response = add_teacher_via_api(
            test_client, auth_headers, fake_org_id, teacher.id, "org_admin"
        )
...
class TestOrganizationStats:
    ...
    def test_get_stats_empty(
        self, test_client: TestClient, shared_test_session: Session
    ):
        ...
        access_token = create_access_token(
            data={"sub": str(new_teacher.id)}, expires_delta=timedelta(minutes=30)
        )
    ...
    def test_get_stats_aggregation(
        ...
        add_teacher_via_api(
            test_client, auth_headers, org1["id"], teacher1.id, "org_admin"
        )
    ...
    def test_get_stats_requires_auth(self, test_client: TestClient):
