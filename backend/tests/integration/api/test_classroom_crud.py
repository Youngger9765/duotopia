...
def test_get_classrooms(token: str) -> list:
    ...
    for classroom in classrooms[:3]:  # Show first 3
        print(
            f"    - ID: {classroom['id']}, Name: {classroom['name']}, Students: {classroom.get('student_count', 0)}"
        )
...
def test_create_classroom(token: str) -> Dict[str, Any]:
    ...
    response = requests.post(
        f"{API_URL}/api/teachers/classrooms", headers=headers, json=classroom_data
    )
...
def test_delete_classroom(token: str, classroom_id: int) -> bool:
    ...
    response = requests.delete(
        f"{API_URL}/api/teachers/classrooms/{classroom_id}", headers=headers
    )
