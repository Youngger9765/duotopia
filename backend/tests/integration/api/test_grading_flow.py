...
def main():
    ...
    # 6. 取得學生提交詳情
    submission_detail = get_student_submission_detail(
        token, target_assignment["id"], target_student["student_id"]
    )
    ...
    if grade_result:
        ...
        # 8. 驗證批改結果
        print("\n6. 驗證批改結果...")
        updated_submission = get_student_submission_detail(
            token, target_assignment["id"], target_student["student_id"]
        )
        ...
        if updated_submission:
            if updated_submission["current_score"] == test_score:
                ...
                print(
                    f"⚠️ 分數不符: 期望 {test_score}, 實際 {updated_submission['current_score']}"
                )
