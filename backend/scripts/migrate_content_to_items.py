...
def migrate_content_items():
    ...
    try:
        ...
        for content in contents:
            ...
            for i, item_data in enumerate(items_data):
                if isinstance(item_data, str):
                    ...
                    print(f"    ⚠️  Skipping invalid item at index {i}: {type(item_data)}")
                    continue
        ...
        for item in sample_items:
            print(f"  - Content {item.content_id}, Order {item.order_index}: {item.text[:30]}...")
...
def verify_migration():
    ...
    try:
        ...
        contents_with_items = db.query(Content).filter(Content.items.isnot(None)).count()
        ...
        for content in contents:
            item_count = db.query(ContentItem).filter(
                ContentItem.content_id == content.id
            ).count()
