...
def check_migration_file(filepath: str) -> tuple[bool, list[str]]:
    ...
    try:
        ...
        # 檢查 down_revision
        down_revision_match = re.search(r"down_revision:\s*.*\s*=\s*['\"]([^'\"]*)['\"]|down_revision:\s*.*\s*=\s*None", content)
        ...
        if upgrade_match:
            ...
            dangerous_patterns = [
                ...
                (r'execute\(.*DELETE', '⚠️  警告: upgrade() 中包含 DELETE 語句 - 請確認是否真的要刪除資料'),
                (r'execute\(.*TRUNCATE', '⚠️  警告: upgrade() 中包含 TRUNCATE 語句 - 請確認是否真的要清空表'),
            ]
