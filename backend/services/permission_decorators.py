...
def require_permission(
    ...
    def decorator(f: Callable):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not teacher_id:
                return jsonify({"error": "Unauthorized", "detail": "No teacher_id in request"}), 401

            # 決定 domain
            domain = _get_domain(teacher_id, domain_param, use_org_domain, kwargs)

            if not domain:
                return jsonify({"error": "Bad Request", "detail": "Cannot determine domain"}), 400

            # 檢查權限
            casbin_service = get_casbin_service()

            if not casbin_service.check_permission(teacher_id, domain, resource, action):
                return jsonify({
    ...
    *roles: str,
    ...
    def decorator(f: Callable):
        ...
        def decorated_function(*args, **kwargs):
            ...
            has_role = any(
                casbin_service.has_role(teacher_id, role, domain)
                for role in roles
            )
    ...
    if domain_param:
        ...
        if domain_param == 'org_id' or domain_param == 'organization_id':
            ...
            if isinstance(domain_value, str):
                if domain_value.startswith('org-') or domain_value.startswith('school-'):
                    return domain_value
                else:
                    # 假設是 UUID
                    return f"school-{domain_value}"
