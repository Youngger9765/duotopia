...
async def cancel_subscription(
    ...
    try:
        ...
        logger.info(f"  subscription_auto_renew: {current_teacher.subscription_auto_renew}")

        # 檢查是否有有效訂閱
        if not current_teacher.subscription_end_date:
            logger.warning(f"Teacher {current_teacher.email} has no subscription_end_date")
        ...
        if end_date < now:
            logger.warning(f"Teacher {current_teacher.email} subscription expired: {end_date} < {now}")
