...
async def get_school_students(
    ...
    if classroom_id is not None:
        query = query.join(ClassroomStudent, Student.id == ClassroomStudent.student_id).filter(
    ...
    if unassigned:
        query = query.outerjoin(ClassroomStudent, Student.id == ClassroomStudent.student_id).filter(
            ClassroomStudent.id.is_(None)
        )
...
@router.get("/api/schools/{school_id}/classrooms/{classroom_id}/students", response_model=List[dict])
...
async def add_student_to_school(
    ...
    existing = db.query(StudentSchool).filter(
                    ...
                    if student_item.classroom_id:
                        if check_classroom_in_school(student_item.classroom_id, school_id, db):
                            enrollment = db.query(ClassroomStudent).filter(
                                ...
                                ClassroomStudent.classroom_id == student_item.classroom_id
                            ).first()
                            if not enrollment:
                                enrollment = ClassroomStudent(
...
async def update_school_student(
    ...
    if update_data.student_number is not None:
        ...
        if update_data.student_number and update_data.student_number != student.student_number:
            existing = (
                db.query(Student)
                .join(StudentSchool, Student.id == StudentSchool.student_id)
                .filter(
...
async def remove_classroom_student(
    ...
    """
    return await remove_student_from_classroom(school_id, student_id, classroom_id, teacher, db)
