...
from models import Student, Classroom, ClassroomStudent, StudentAssignment, AssignmentStatus
...
@router.get("/profile")
async def get_student_profile(current_user: Dict[str, Any] = Depends(get_current_user), db: Session = Depends(get_db)):
    """取得當前學生資訊"""
...
async def get_student_assignments(current_user: Dict[str, Any] = Depends(get_current_user), db: Session = Depends(get_db)):
    """取得學生作業列表"""
    if current_user.get("type") != "student":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    # Get assignments
    assignments = db.query(StudentAssignment).filter(
