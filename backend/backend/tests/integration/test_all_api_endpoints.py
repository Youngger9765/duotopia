...
class TestAPIEndpoints:
    ...
    def test_assignment_endpoints(self):
        ...
        if assignments:
            ...
            response = client.get(f"/api/teachers/assignments/{assignment_id}", headers=headers)
            assert response.status_code == 200, "Assignment details should be accessible"
    ...
    def test_router_prefixes(self):
        ...
        for pattern in bad_patterns:
            matching_routes = [r for r in routes if r.startswith(pattern)]
            assert len(matching_routes) == 0, f"Found incorrect route pattern {pattern}: {matching_routes}"
