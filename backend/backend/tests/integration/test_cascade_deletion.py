...
def test_cascade_delete():
    ...
    try:
        ...
        program_contents = db.query(Content).join(Lesson).filter(Lesson.program_id == program_id).count()
        ...
        # 檢查相關的 Lessons 是否被刪除
        remaining_lessons = db.query(Lesson).filter(Lesson.program_id == program_id).count()
        remaining_contents = db.query(Content).filter(Content.lesson_id.in_(lesson_ids)).count() if lesson_ids else 0

        print("\n刪除後狀態:")
        print(f"  - Programs: {programs_count} → {new_programs_count} (減少 {programs_count - new_programs_count})")
        print(f"  - Lessons: {lessons_count} → {new_lessons_count} (減少 {lessons_count - new_lessons_count})")
        print(f"  - Contents: {contents_count} → {new_contents_count} (減少 {contents_count - new_contents_count})")
        ...
        if remaining_lessons == 0 and remaining_contents == 0:
            ...
            for row in result:
                print(f"  - {row.table_name}.{row.column_name}: DELETE {row.delete_rule}")
