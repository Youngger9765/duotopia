...
class TestFeedbackSeparation:
    ...
    def test_save_detailed_record_and_overall_comment(self, test_grading_setup, teacher_token, db: Session):
        """測試儲存詳實記錄和總評"""
        ...
        # 驗證儲存的資料
        student_assignment = db.query(StudentAssignment).filter_by(assignment_id=1, student_id=1).first()
    ...
    def test_detailed_record_without_overall_comment(self, test_grading_setup, teacher_token, db: Session):
        """測試只有詳實記錄，沒有總評的情況"""
        headers = {"Authorization": f"Bearer {teacher_token}"}
    ...
    def test_item_feedback_persistence(self, test_grading_setup, teacher_token, db: Session):
        """測試各題評語的持久性"""
        headers = {"Authorization": f"Bearer {teacher_token}"}
        ...
        # 驗證各題評語已儲存
        progress_1 = db.query(StudentContentProgress).filter_by(student_assignment_id=1, content_id=1).first()
        assert progress_1.feedback == "第一題評語"
        assert progress_1.checked.is_(True)

        progress_3 = db.query(StudentContentProgress).filter_by(student_assignment_id=1, content_id=3).first()
        ...
        progress_2 = db.query(StudentContentProgress).filter_by(student_assignment_id=1, content_id=2).first()
