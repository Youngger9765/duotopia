...
def test_student_get_assignment_detail():
    ...
    response = requests.get(f"{BASE_URL}/assignments/{assignment_id}/detail", headers=student_headers)
...
def test_student_submit_assignment():
    ...
    unfinished = next((a for a in assignments if a["status"] in ["NOT_STARTED", "IN_PROGRESS"]), None)
...
def test_teacher_view_submissions():
    ...
    # 2. 獲取班級作業
    print("\n2. 獲取班級作業...")
    response = requests.get(f"{BASE_URL}/assignments/teacher?classroom_id=1", headers=teacher_headers)
    ...
    # 3. 查看提交詳情
    print("\n3. 查看提交詳情...")
    response = requests.get(f"{BASE_URL}/assignments/{assignment_id}/submissions", headers=teacher_headers)
...
def test_teacher_manual_grade():
    ...
    assignments = response.json()
    submitted = next((a for a in assignments if a.get("submissions_count", 0) > 0), None)
