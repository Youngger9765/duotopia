...
def teacher_with_data(db: Session):
    ...
    for i in range(3):
        ...
        # Add to classroom
        cs = ClassroomStudent(classroom_id=classroom.id, student_id=student.id, is_active=True)
        db.add(cs)
...
class TestTeacherClassroomCRUD:
    ...
    def test_update_classroom_success(self, db: Session, teacher_with_data: Teacher, coverage_teacher_token: str):
        """Test successful classroom update"""
        # Get existing classroom
        classroom = db.query(Classroom).filter(Classroom.teacher_id == teacher_with_data.id).first()
    ...
    def test_delete_classroom_success(self, db: Session, teacher_with_data: Teacher, coverage_teacher_token: str):
    ...
    def test_get_classroom_detail(self, db: Session, teacher_with_data: Teacher, coverage_teacher_token: str):
...
class TestTeacherStudentCRUD:
    ...
    def test_batch_add_students(self, db: Session, teacher_with_data: Teacher, coverage_teacher_token: str):
    ...
    def test_update_student_success(self, db: Session, teacher_with_data: Teacher, coverage_teacher_token: str):
        """Test successful student update"""
        # Get a student from the classroom
        classroom = db.query(Classroom).filter(Classroom.teacher_id == teacher_with_data.id).first()

        student_relation = db.query(ClassroomStudent).filter(ClassroomStudent.classroom_id == classroom.id).first()
...
class TestTeacherProgramCRUD:
    ...
    def test_create_program_success(self, db: Session, teacher_with_data: Teacher, coverage_teacher_token: str):
    ...
    def test_update_program_success(self, db: Session, teacher_with_data: Teacher, coverage_teacher_token: str):
        """Test successful program update"""
        program = db.query(Program).filter(Program.teacher_id == teacher_with_data.id).first()
    ...
    def test_delete_program_success(self, db: Session, teacher_with_data: Teacher, coverage_teacher_token: str):
...
class TestAuthorizationAndErrors:
    ...
    def test_invalid_token_format(self):
        ...
        response = client.get("/api/teachers/me", headers={"Authorization": "InvalidToken"})
        assert response.status_code == 401
    ...
    def test_expired_token(self):
        ...
        response = client.get("/api/teachers/me", headers={"Authorization": f"Bearer {expired_token}"})
        assert response.status_code == 401
...
class TestInputValidation:
    ...
    def test_duplicate_email_student(self, db: Session, teacher_with_data: Teacher, coverage_teacher_token: str):
