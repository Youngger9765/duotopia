...
def test_students(test_db: Session, test_classroom):
    ...
    for i in range(3):
        ...
        # Add to classroom
        enrollment = ClassroomStudent(classroom_id=test_classroom.id, student_id=student.id, is_active=True)
...
class TestAssignmentCreation:
    ...
    def test_create_assignment_for_individual_student(
        ...
        student_assignments = (
            test_db.query(StudentAssignment).filter_by(student_id=student.id, assignment_id=result["id"]).all()
        )
    ...
    def test_create_assignment_for_multiple_students(
        ...
        student_assignments = test_db.query(StudentAssignment).filter_by(assignment_id=result["id"]).all()
    ...
    def test_create_assignment_validation_errors(self, test_classroom, test_students, test_content, auth_headers):
        """測試作業創建的驗證錯誤"""
        ...
        assert response.status_code == 400, "Should return student not in classroom error"

    def test_create_assignment_with_past_due_date(self, test_classroom, test_students, test_content, auth_headers):
        """測試使用過去日期作為截止日期"""
        assignment_data = {
            "title": "過期作業測試",
    ...
    def test_duplicate_assignment_handling(self, test_db, test_classroom, test_students, test_content, auth_headers):
        """測試重複作業的處理"""
        student = test_students[0]
        ...
        response1 = client.post("/api/assignments", json=assignment_data, headers=auth_headers)
        ...
        response2 = client.post("/api/assignments", json=assignment_data, headers=auth_headers)
