    ...
    async def test_generate_tts_success(self, tts_service):
        ...
        with patch("edge_tts.Communicate") as mock_communicate:
            ...
            with patch.object(tts_service, "_get_storage_client") as mock_storage:
                ...
                result = await tts_service.generate_tts(text="Hello World", voice="en-US-JennyNeural")
        ...
        with patch("edge_tts.Communicate") as mock_communicate:
            ...
            with patch.object(tts_service, "_get_storage_client") as mock_storage:
                ...
                with pytest.raises(Exception) as exc_info:
                    await tts_service.generate_tts(text="Hello World", voice="en-US-JennyNeural")
...
class TestTranslationService:
    ...
    async def test_translate_text_to_chinese(self, translation_service_fixture):
        ...
        with patch.object(translation_service_fixture.client.chat.completions, "create") as mock_create:
            mock_response = Mock()
    ...
    @pytest.mark.asyncio
    async def test_translation_error_returns_original(self, translation_service_fixture):
...
class TestAudioUploadService:
    ...
    async def test_upload_audio_success(self, upload_service):
        ...
        with patch.object(upload_service, "_get_storage_client") as mock_storage:
            ...
            result = await upload_service.upload_audio(file=mock_file, duration_seconds=5, content_id=123, item_index=0)

            assert result.startswith("https://storage.googleapis.com/")
            assert "recording_c123_i0_" in result
            assert ".webm" in result
            mock_blob.upload_from_string.assert_called_once_with(audio_content, content_type="audio/webm")
    ...
    async def test_upload_audio_duration_limit(self, upload_service):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await upload_service.upload_audio(file=mock_file, duration_seconds=35)  # Over 30 second limit
...
class TestAudioAPIEndpoints_Skip:
    ...
    def test_generate_tts_endpoint(self, auth_headers, mock_teacher):
        ...
        with patch("routers.teachers.get_current_teacher") as mock_auth:
            ...
            with patch("routers.teachers.get_tts_service") as mock_service_getter:
                mock_service = Mock()
                mock_service.generate_tts = AsyncMock(return_value="https://storage.googleapis.com/test.mp3")
    ...
    def test_upload_audio_endpoint(self, auth_headers, mock_teacher):
        ...
        with patch("routers.teachers.get_current_teacher") as mock_auth:
            ...
            with patch("routers.teachers.get_audio_upload_service") as mock_service_getter:
                mock_service = Mock()
                mock_service.upload_audio = AsyncMock(return_value="https://storage.googleapis.com/test.webm")
                mock_service_getter.return_value = mock_service
    ...
    async def test_tts_to_upload_flow(self):
        ...
        with patch("edge_tts.Communicate") as mock_communicate:
            ...
            with patch.object(tts_service, "_get_storage_client") as mock_tts_storage:
                with patch.object(upload_service, "_get_storage_client") as mock_upload_storage:
                    ...
                    # Generate TTS
                    tts_url = await tts_service.generate_tts("Test", "en-US-JennyNeural")
