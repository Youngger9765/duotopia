...
class TestAssignmentLists:
    ...
    def test_teacher_get_all_assignments(self):
        """測試教師查詢所有作業（不指定班級）"""
        response = self.client.get("/api/assignments/teacher", headers=self.teacher_headers)
    ...
    def test_student_assignment_filtering(self):
        ...
        # 測試狀態篩選（如果 API 支援）
        response = self.client.get("/api/assignments/student?status=pending", headers=student_headers)
    ...
    def test_invalid_requests(self):
        """測試無效請求的錯誤處理"""
        # 測試 1: 教師查詢不存在的班級
        response = self.client.get("/api/assignments/teacher?classroom_id=99999", headers=self.teacher_headers)
