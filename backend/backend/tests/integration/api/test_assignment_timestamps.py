...
def test_content(test_db, test_teacher, test_classroom):
    ...
    lesson = Lesson(program_id=program.id, name="Test Lesson", order_index=1, is_active=True)
    test_db.add(lesson)
    test_db.flush()
...
def test_assignment(test_db, test_teacher, test_classroom, test_content):
    ...
    # Link content
    assignment_content = AssignmentContent(assignment_id=assignment.id, content_id=test_content.id, order_index=1)
...
def test_assignment_timestamps(test_db, test_assignment, test_students, auth_headers):
    ...
    returned_time = datetime.fromisoformat(s2_data["timestamps"]["returned_at"].replace("+00:00", ""))
    resubmitted_time = datetime.fromisoformat(s2_data["timestamps"]["resubmitted_at"].replace("+00:00", ""))
    ...
    returned_time = datetime.fromisoformat(s3_data["timestamps"]["returned_at"].replace("+00:00", ""))
    resubmitted_time = datetime.fromisoformat(s3_data["timestamps"]["resubmitted_at"].replace("+00:00", ""))
    graded_time = datetime.fromisoformat(s3_data["timestamps"]["graded_at"].replace("+00:00", ""))
    assert resubmitted_time > returned_time
