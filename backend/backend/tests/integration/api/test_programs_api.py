...
def test_teacher(db: Session):
    ...
    existing_teacher = db.query(Teacher).filter(Teacher.email == "test_programs_teacher@example.com").first()
    if existing_teacher:
        # 取得所有相關的 Programs
        programs = db.query(Program).filter(Program.teacher_id == existing_teacher.id).all()
...
def test_classroom_2(db: Session, test_teacher):
    """建立第二個測試用班級"""
    classroom = Classroom(name="Another Classroom", teacher_id=test_teacher.id, level="B1")
    db.add(classroom)
    db.commit()
    db.refresh(classroom)
    return classroom
...
class TestProgramTemplates:
    ...
    def test_get_template_programs(self, client: TestClient, db: Session, auth_headers, test_teacher):
        """測試取得公版課程模板列表"""
        # 建立幾個公版課程
        for i in range(3):
            program = Program(
...
class TestProgramCopy:
    ...
    def test_create_custom_program(self, client: TestClient, auth_headers, test_classroom):
        """測試在班級中自建課程"""
        response = client.post(
...
class TestProgramManagement:
    ...
    def test_get_classroom_programs(
        ...
        response = client.get(f"/api/programs/classroom/{test_classroom.id}", headers=auth_headers)
    ...
    def test_soft_delete_program(self, client: TestClient, db: Session, auth_headers, test_teacher):
        """測試軟刪除課程"""
        # 建立課程
        program = Program(name="To Be Deleted", is_template=True, teacher_id=test_teacher.id)
        db.add(program)
        db.commit()
        db.refresh(program)
    ...
    def test_get_copyable_programs(
        ...
        """測試取得可複製的課程列表"""
        # 建立公版模板
        template = Program(name="Template", is_template=True, teacher_id=test_teacher.id)
        db.add(template)
