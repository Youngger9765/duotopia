...
def create_program(token, classroom_id):
    ...
    response = requests.post(f"{API_URL}/api/teachers/programs", headers=headers, json=program_data)
...
def main():
    ...
    if classrooms:
        ...
        if program:
            ...
            updated = next((c for c in updated_classrooms if c["id"] == classroom["id"]), None)
