...
class TestManualGrading:
    ...
    def _create_graded_assignment_for_manual_grading(self):
        ...
        response = self.client.post("/api/teachers/programs", json=program_data, headers=self.teacher_headers)
    ...
    def _get_student_token(self, email, password):
        """取得學生 token"""
        response = self.client.post("/api/auth/student/login", json={"email": email, "password": password})
        if response.status_code != 200:
            return None
    ...
    def test_manual_feedback_editing(self):
        ...
        response = self.client.get(f"/api/assignments/{assignment_id}/detail", headers=student_headers)
    ...
    def test_assignment_comparison_view(self):
        ...
        if "score_differences" in comparison:
            ...
            assert "ai_score" in score_diff or "manual_score" in score_diff or "difference" in score_diff
    ...
    def test_manual_grading_permissions(self):
        ...
        # 測試未認證請求
        response = self.client.patch(f"/api/assignments/{assignment_id}/manual-grade", json=manual_adjustment)
