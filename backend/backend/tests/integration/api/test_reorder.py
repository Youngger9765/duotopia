...
def test_reorder_programs():
    ...
    assert reorder_response.status_code == 200, f"重新排序失敗: {reorder_response.status_code}, {reorder_response.text}"
    print("✅ 課程重新排序成功")

    # 4. 驗證新順序（重新取得課程列表）
    verify_response = requests.get("http://localhost:8000/api/teachers/programs", headers=headers)
...
def test_reorder_lessons():
    ...
    # 記錄原始順序
    original_order = [(lesson["id"], lesson.get("order_index", 0)) for lesson in lessons[:2]]
