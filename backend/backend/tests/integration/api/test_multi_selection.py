...
def test_get_templates(token):
    ...
    if response.status_code == 200:
        ...
        for i, template in enumerate(templates[:3]):
            print(f"   {i+1}. {template['name']} (重複: {template.get('is_duplicate', False)})")
...
def test_multi_copy_templates(token):
    ...
    # 取得模板
    templates = requests.get(f"{BASE_URL}/api/programs/templates", headers=headers).json()
...
def main():
    ...
    test_get_copyable_programs(token)
