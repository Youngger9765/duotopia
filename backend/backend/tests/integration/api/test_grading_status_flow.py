...
class TestGradingStatusFlow:
    ...
    def test_save_grade_without_status_change(self, test_assignment_setup, teacher_token):
        ...
        # 確認狀態仍然是 SUBMITTED
        response = client.get("/api/teachers/assignments/1/submissions/1", headers=headers)
        assert response.status_code == 200
        submission = response.json()
        assert submission["status"] == "SUBMITTED"

    def test_complete_grading_with_status_change(self, test_assignment_setup, teacher_token):
    ...
    def test_save_item_results_with_feedback(self, test_assignment_setup, teacher_token):
        """測試儲存個別題目評分和評語"""
        headers = {"Authorization": f"Bearer {teacher_token}"}
    ...
    def test_unauthorized_teacher_cannot_grade(self, test_assignment_setup):
        """測試未授權教師無法批改作業"""
        # 創建另一個教師的 token
        other_teacher_token = create_access_token(data={"sub": "other_teacher@example.com", "user_type": "teacher"})
