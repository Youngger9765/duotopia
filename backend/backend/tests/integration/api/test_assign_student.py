...
def get_assignment_detail(token, assignment_id):
    ...
    response = requests.get(f"{BASE_URL}/api/assignments/{assignment_id}", headers=headers)
    if response.status_code == 200:
...
def get_classroom_students(token, classroom_id):
    """取得班級學生列表"""
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/classrooms/{classroom_id}/students", headers=headers)
    if response.status_code == 200:
        return response.json()
    else:
...
def get_assignment_progress(token, assignment_id):
    """取得作業進度（檢查哪些學生被指派）"""
    headers = {"Authorization": f"Bearer {token}"}
    response = requests.get(f"{BASE_URL}/api/assignments/{assignment_id}/progress", headers=headers)
    if response.status_code == 200:
        return response.json()
    else:
    ...
    if success:
        ...
        if updated_assignment:
            ...
            if progress and "students" in progress:
                assigned_count = len([s for s in progress["students"] if s.get("is_assigned")])
                ...
                for student in progress["students"][:8]:
                    ...
                    print(f"   {status} {student['name']} (ID: {student['id']}) - 狀態: {assignment_status}")
