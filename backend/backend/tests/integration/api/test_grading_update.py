...
def test_grading_update():
    ...
    if verify_response.status_code == 200:
        ...
        assert data.get("current_score") == first_score, f"分數不符: {data.get('current_score')} != {first_score}"
    ...
    if final_verify.status_code == 200:
        ...
        assert data.get("current_score") == second_score, f"分數未更新: {data.get('current_score')} != {second_score}"
