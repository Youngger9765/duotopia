...
def test_template_duplicate_detection():
    ...
    # 1. 獲取教師的班級
    print("1️⃣ 獲取教師班級...")
    classrooms_response = requests.get(f"{BASE_URL}/api/teachers/classrooms", headers=headers)
    ...
    for template in templates:
        print(f"   - 模板「{template['name']}」: is_duplicate = {template.get('is_duplicate', 'None')}")
        ...
        if copy_response.status_code == 200:
            ...
            for template in templates_after:
                ...
                if template["id"] == template_to_copy["id"]:
                    print(f"   - 已複製模板「{template['name']}」: is_duplicate = {is_dup} {'✅' if is_dup else '❌'}")
                    if is_dup:
                        print("✅ 重複檢測成功！")
                        return True
                    else:
...
def test_classroom_duplicate_detection():
    ...
    for program in copyable_programs:
        ...
        print(f"   - 課程「{program['name']}」來自「{classroom_name}」: is_duplicate = {is_dup}")
