...
def test_students(test_db: Session, test_classroom):
    ...
    for i in range(5):
        ...
        # Add to classroom
        classroom_student = ClassroomStudent(classroom_id=test_classroom.id, student_id=student.id, is_active=True)
...
class TestAssignmentAPI:
    ...
    def test_get_assignment_detail(self, client, test_assignment, auth_headers):
        """測試取得作業詳情"""
        response = client.get(f"/api/assignments/{test_assignment.id}", headers=auth_headers)
    ...
    def test_patch_assignment_student_ids(self, client, test_assignment, test_students, auth_headers, test_db):
        """測試更新作業的學生指派"""
        ...
        student_assignments = (
            test_db.query(StudentAssignment).filter(StudentAssignment.assignment_id == test_assignment.id).all()
        )
    ...
    def test_patch_assignment_remove_students(self, client, test_assignment, test_students, auth_headers, test_db):
        """測試移除學生指派"""
    ...
    def test_patch_assignment_multiple_fields(self, client, test_assignment, test_students, auth_headers):
        """測試同時更新多個欄位"""
        new_data = {
    ...
    def test_get_assignment_progress(self, client, test_assignment, test_students, auth_headers, test_db):
        """測試取得作業進度"""
        ...
        # Get progress
        response = client.get(f"/api/assignments/{test_assignment.id}/progress", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_classroom_students(self, client, test_classroom, test_students, auth_headers):
        """測試取得班級學生列表"""
        response = client.get(f"/api/classrooms/{test_classroom.id}/students", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_patch_preserves_started_assignments(self, client, test_assignment, test_students, auth_headers, test_db):
        """測試更新時保留已開始的作業"""
        # Create assignments with different statuses
        for i, student in enumerate(test_students[:3]):
            status = AssignmentStatus.IN_PROGRESS if i == 0 else AssignmentStatus.NOT_STARTED
            sa = StudentAssignment(
                assignment_id=test_assignment.id,
        ...
        # Verify total count
        total = test_db.query(StudentAssignment).filter(StudentAssignment.assignment_id == test_assignment.id).count()
        assert total == 2
