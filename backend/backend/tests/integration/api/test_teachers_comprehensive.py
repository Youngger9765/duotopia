...
class TestDataIntegrityAndEdgeCases:
    ...
    def test_empty_classrooms_handled_correctly(
        ...
        empty_classroom_data = next((c for c in data["classrooms"] if c["name"] == "Empty Classroom"), None)
    ...
    def test_null_descriptions_handled(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers, db):
        ...
        null_desc_data = next((c for c in data["classrooms"] if c["name"] == "Null Desc Classroom"), None)
    ...
    def test_classroom_student_relationship_integrity(
        ...
        student = db.query(Student).first()
        classroom = (
            db.query(Classroom).filter(Classroom.teacher_id == teacher_with_comprehensive_data.id).all()[-1]
        )  # Last classroom

        # Add student to additional classroom
        additional_assignment = ClassroomStudent(classroom_id=classroom.id, student_id=student.id, is_active=True)
    ...
    def test_recent_students_limit(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers, db):
        ...
        students = []
        classrooms = db.query(Classroom).filter(Classroom.teacher_id == teacher_with_comprehensive_data.id).all()
...
class TestErrorRecovery:
    ...
    def test_concurrent_modifications(self, client, teacher_with_comprehensive_data, comprehensive_auth_headers, db):
        ...
        # Modify data
        classroom = db.query(Classroom).filter(Classroom.teacher_id == teacher_with_comprehensive_data.id).first()
        ...
        data = response2.json()
        modified_classroom = next((c for c in data["classrooms"] if c["name"] == "Modified Name"), None)
