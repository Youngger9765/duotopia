...
def test_complete_grading_flow():
    ...
    # 2. 取得作業列表
    print("\n2. 取得作業列表...")
    assignments_response = requests.get(f"{BASE_URL}/api/teachers/assignments", headers=headers)
    ...
    # 3. 取得學生提交列表
    print("\n3. 取得學生提交列表...")
    submissions_response = requests.get(f"{BASE_URL}/api/teachers/{assignment_id}/submissions", headers=headers)
    ...
    print(f"   選擇學生: {student_to_grade['student_name']} (ID: {student_id}, 狀態: {student_to_grade['status']})")
    ...
    if verify_response.status_code == 200:
        ...
        if updated_submission.get("current_score") == test_score:
            ...
            print(f"⚠️ 分數不符: 期望 {test_score}, 實際 {updated_submission.get('current_score')}")
