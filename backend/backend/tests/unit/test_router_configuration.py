        ...
        for module_name, expected in expected_configs.items():
            try:
                ...
                if actual_prefix != expected["prefix"]:
                    errors.append(
                        f"{module_name}: prefix 錯誤 - " f"預期 '{expected['prefix']}', 實際 '{actual_prefix}'"
                    )

                # 檢查 tags
                actual_tags = getattr(router, "tags", [])
                if actual_tags != expected["tags"]:
                    errors.append(f"{module_name}: tags 錯誤 - " f"預期 {expected['tags']}, 實際 {actual_tags}")
...
def test_critical_api_flow():
    ...
    assert assignments_response.status_code == 200, f"無法取得作業列表: {assignments_response.status_code}"
    ...
    if assignments:
        ...
        detail_response = client.get(f"/api/teachers/assignments/{assignment_id}", headers=headers)
        assert detail_response.status_code == 200, f"無法取得作業詳情: {detail_response.status_code}"
