...
def downgrade() -> None:
    ...
    op.drop_index('ix_teacher_organizations_composite', table_name='teacher_organizations')

    # Drop GIN indexes (PostgreSQL only)
    if op.get_bind().dialect.name == 'postgresql':
        op.drop_index('ix_teacher_schools_permissions_gin', table_name='teacher_schools')
