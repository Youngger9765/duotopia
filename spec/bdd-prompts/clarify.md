...
1. **讀取釐清策略**：根據當前工作的模組，載入對應的 `overview.md`：
   - **機構模組**：`spec/.clarify/overview.md`
   - **個人教師模組**：`spec/individual-teacher/.clarify/overview.md`
...
2. **載入釐清項目**：依照 `overview.md` 建議的順序，讀取對應模組下的所有釐清項目檔案：
   - **機構模組**：`spec/.clarify/data/*.md` 和 `spec/.clarify/features/*.md`
   - **個人教師模組**：`spec/individual-teacher/.clarify/data/*.md` 和 `spec/individual-teacher/.clarify/features/*.md`

3. **載入現有規格**：根據當前工作的模組，載入對應的規格檔案：

   **機構模組**：
   - DBML 檔案：`spec/erm-organization.dbml`, `spec/erm-subscription.dbml` 等
   - Feature 檔案：`spec/features/organization/*.feature`, `spec/features/subscription/*.feature` 等

   **個人教師模組**：
   - DBML 檔案：`spec/individual-teacher/erm-subscription.dbml` 等（若存在）
   - Feature 檔案：`spec/individual-teacher/features/subscription/*.feature` 等
   - 保持原始格式與結構,供後續增量更新使用
   - **建立術語對照表**：提取所有繁體中文術語（Table 名稱的中文 note、欄位的中文 note、Feature 名稱、Scenario/Rule 描述、Given/When/Then 步驟用詞）,供翻譯時參照
...
1. **術語一致性檢查**：
   - 載入對應的 DBML 檔案（`spec/erm-core.dbml` 等 5 個領域檔案）和 `spec/features/<domain>/*.feature` 中該實體/功能的相關區段
   - 提取已使用的繁體中文術語（與步驟 0.3 相同：Table 名稱的中文 note、欄位的中文 note、Feature 名稱、Scenario/Rule 描述、Given/When/Then 步驟用詞）
   - 參考 `../../../spec/business-specs/student-abilities-assessment.md` 和 `../../../spec/business-specs/user-roles-and-permissions.md` 了解業務背景術語
   - 建立「技術詞彙 ↔ 規格中文術語」對照表
...
> `StudentItemProgress` 實體的 `pronunciation_score` 屬性是否允許 `null` 值？

**翻譯後問題**：

> 學生朗讀紀錄的發音分數 (pronunciation_score) 是否可以空白（未設定）？
...
根據釐清項目的定位與所屬模組，決定更新目標：

**機構模組釐清**：

- **資料模型釐清** → 更新 `spec/erm-organization.dbml`, `spec/erm-subscription.dbml` 等，依照 `formulation-rules.md` 的 DBML 格式規範
  - 更新實體屬性定義（型別、note、約束）
  - 新增或修改關係定義
  - 更新跨屬性不變條件
  - 新增狀態轉換規則

- **功能模型釐清** → 更新 `spec/features/<domain>/<功能名>.feature`（依照 `formulation-rules.md` 的 Gherkin Language 格式規範）
  - 新增或修改 Rule
...
- **資料模型釐清** → 更新 `spec/individual-teacher/erm-subscription.dbml` 等，依照 `formulation-rules.md` 的 DBML 格式規範
  - 更新實體屬性定義（型別、note、約束）
  - 新增或修改關係定義
  - 更新跨屬性不變條件
  - 新增狀態轉換規則

- **功能模型釐清** → 更新 `spec/individual-teacher/features/<domain>/<功能名>.feature`（依照 `formulation-rules.md` 的 Gherkin Language 格式規範）
  - 新增或修改 Rule
  - 新增或修改 Example（遵循 Given-When-Then 格式）
  - 更新前置/後置條件
  - 補充錯誤處理情境
...
**原始釐清問題**（技術導向）：「StudentItemProgress 的 pronunciation_score 是否允許負值？」

**翻譯後問題**（白話）：「學生朗讀紀錄的發音分數 (pronunciation_score) 是否可以是負數？」

**使用者答案**：「A - 不可以，必須是 0 到 100 之間的分數」

**規格更新**（檔案：`spec/erm-classroom.dbml`）：
...
Table StudentItemProgress {
  id int [pk]
  pronunciation_score float [note: "發音分數"]
}
```
...
Table StudentItemProgress {
  id int [pk]
  pronunciation_score float [note: "發音分數，範圍 0-100"]
}
```

### 3.5 Gherkin 更新範例

**原始釐清問題**（技術導向）：「提交朗讀作業時，pronunciation_score 低於 60 如何處理？」

**翻譯後問題**（白話）：「學生提交朗讀作業時，發音分數 (pronunciation_score) 低於 60 分該怎麼處理？」

**使用者答案**：「B - 要求重新錄製，並顯示提示訊息」

**規格更新**（檔案：`spec/features/classroom/提交朗讀作業.feature`）：
...
Feature: 提交朗讀作業
  ...
  Rule: 發音分數低於 60 分時要求重新錄製
    Example: 拒絕低分朗讀作業
      Given 學生「小明」已登入
      And 朗讀作業「Unit 1 - Sentence 1」存在
      When 學生提交朗讀錄音檔，發音分數為 45
      Then 系統顯示提示訊息「發音分數過低，請重新錄製」
      And 作業狀態標記為「待重新提交」
```
...
- **更新的規格檔**：<spec/erm-_.dbml 或 spec/features/<domain>/_.feature>
- **變更內容**：<簡述規格檔的變更內容>
```
...
- **回答**：B - 班級必須屬於某位教師（Teacher 1:N Classroom）
- **更新的規格檔**：spec/erm-classroom.dbml
- **變更內容**：在 Classroom 實體中新增 teacher_id 外鍵，建立 Teacher 1:N Classroom 關係
```
...
- [ ] 不存在互相矛盾的陳述
- [ ] 格式有效：
  - 對應的 `spec/erm-*.dbml` 檔案遵守 DBML 語法
  - `spec/features/<domain>/*.feature` 遵守 Gherkin 語法（嚴格階層 Feature > Rule > Example）
...
- `spec/erm-classroom.dbml` - 更新 Classroom, StudentItemProgress 實體
- `spec/erm-core.dbml` - 更新 Teacher, Student 實體
- `spec/features/classroom/提交朗讀作業.feature` - 新增 2 條規則與 3 個例子
- `spec/features/classroom/建立班級.feature` - 更新 1 條規則與 2 個例子
...
1. **互動提問** → 終端輸出或聊天介面
2. **規格更新** → 對應的 `spec/erm-*.dbml` 檔案和 `spec/features/<domain>/*.feature`
