...
- 修改 email 驗證後的自動開通邏輯
- 更新測試數據
- **遷移現有 30-Day Trial 訂閱**：所有現有的 30-Day Trial 訂閱將自動轉換為 Point-Based Trial
- **不修改資料表結構**
...
- 個人教師訂閱頁需顯示方案名稱（"30-Day Trial" 或 "Point-Based Trial"）
- Admin 後台訂閱管理頁需顯示方案名稱與差異
- 當資料中存在兩種方案時，UI 需正確區分顯示

### 4. 升級付費點數轉移

- 升級付費時，Point-Based Trial 剩餘點數全額轉移至新訂閱
- 轉移後 Trial 訂閱狀態標記為 cancelled
...
**檔案：** `backend/migrations/migrate_30day_trial_to_point_based.py`（新建）

**目的：** 將所有現有的 30-Day Trial 訂閱轉換為 Point-Based Trial
...
def migrate_30day_to_point_based(db):
    """
    將所有現有的 30-Day Trial 訂閱轉換為 Point-Based Trial
    """
    # 更新所有 plan_name = "30-Day Trial" 的訂閱
    db.execute(
        update(SubscriptionPeriod)
        .where(SubscriptionPeriod.plan_name == "30-Day Trial")
        .values(
            plan_name="Point-Based Trial",
            quota_total=4000,  # 統一配額為 4000 秒
            end_date=datetime(2099, 12, 31, 23, 59, 59, tzinfo=timezone.utc)  # 永久有效
        )
    )
    db.commit()
...
**執行時機：** 部署新版本前執行此 migration

---

### 修改檔案 5：配額檢查邏輯（需確認具體檔案）

**目的：** 當配額用完時，API 回應應包含明確的錯誤訊息

**可能位置：**

- `backend/services/quota_service.py`（若存在）
- 或相關的內容創建 API endpoints

**實作邏輯：**

```python
def check_quota_available(teacher_id):
    """
    檢查教師配額是否足夠
    返回 (is_available: bool, quota_info: dict)
    """
    subscription = get_active_subscription(teacher_id)

    if not subscription:
        return False, {"error": "無有效訂閱"}

    if subscription.quota_used >= subscription.quota_total:
        return False, {
            "error": "配額已用完",
            "message": "您的試用配額已用完，請升級付費方案以繼續使用",
            "quota_used": subscription.quota_used,
            "quota_total": subscription.quota_total,
            "plan_name": subscription.plan_name,
            "upgrade_url": "/subscription/upgrade"  # 前端升級頁面路徑
        }

    return True, {
        "quota_used": subscription.quota_used,
        "quota_total": subscription.quota_total,
        "quota_remaining": subscription.quota_total - subscription.quota_used
    }
```

**API 回應格式：**

當操作因配額不足被拒絕時，回應 HTTP 403：

```json
{
  "success": false,
  "error": "QUOTA_EXCEEDED",
  "message": "配額已用完，請升級付費方案",
  "quota_info": {
    "quota_used": 4000,
    "quota_total": 4000,
    "plan_name": "Point-Based Trial",
...
2. **資料遷移測試**
   - 在測試資料庫中創建多個 30-Day Trial 訂閱（包括不同狀態：active、expired）
   - 執行 migration 腳本
   - 驗證所有訂閱：
     - ✅ `plan_name` 已更新為 "Point-Based Trial"
     - ✅ `quota_total` 已更新為 4000
     - ✅ `end_date` 已更新為 2099-12-31
     - ✅ 其他欄位（quota_used、status 等）保持不變

3. **配額限制測試**
   - 使用試用用戶創建內容消耗配額
   - 驗證配額扣除邏輯正常
   - 當 `quota_used >= quota_total` 時：
     - ✅ 操作被拒絕
     - ✅ API 回應包含錯誤訊息與配額狀態
     - ✅ 前端顯示即時提示：「配額已用完，請升級付費方案」
     - ✅ 提示中包含升級按鈕或連結

4. **遷移後的付費訂閱兼容性測試**
   - 確認遷移後，已有的付費訂閱繼續正常運作
   - 確認 Admin 仍可手動創建各種方案訂閱
...
- [x] **所有現有 30-Day Trial 訂閱已自動遷移為 Point-Based Trial**
- [x] Migration 腳本執行成功，無資料遺失
- [x] 遷移後的訂閱 quota_used 保持原值
- [x] Admin 可繼續手動創建任何方案
- [x] UI 正常顯示配額和到期日
- [x] 個人教師 UI 與 Admin 後台可正確顯示方案名稱
- [x] 升級付費時剩餘點數可全額轉移
- [x] 所有相關測試通過
- [x] 種子數據更新完畢
...
- ✅ 資料遷移（新增 migration 腳本）
- ✅ 現有 30-Day Trial 訂閱記錄（全部遷移）
...
- 需要執行 migration 腳本，涉及批量資料更新
- **緩解措施**：
  - 部署前在 staging 環境完整測試 migration
  - 執行前備份 subscription_periods 資料表
  - Migration 腳本應具備 rollback 機制
...
- **Migration 執行順序**：必須在部署新代碼前執行 migration
- 確保 datetime 導入正確
- 驗證 timezone 設定無誤（UTC）
- 測試 seed_data 重新運行
- 新試用用戶不發送額外通知（依 UI 提示/帮助文檔）

---

## 📝 實作 Checklist

- [ ] **建立 migration 腳本** `backend/migrations/migrate_30day_trial_to_point_based.py`
- [ ] 在 staging 環境測試 migration（含 rollback 測試）
