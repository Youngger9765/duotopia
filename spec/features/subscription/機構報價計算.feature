...
功能: 機構報價計算
  ...
  背景:
    ...
    而且 報價計算機已啟用
    而且 點數單價為 "0.0015" TWD
    # 報價計算機不含贈送教師數量，僅以使用者輸入的教師授權數（purchased_teachers）計算，合約建立時才納入 total_teacher_licenses（含贈送教師數量）
    # 預估點數僅供參考，實際購買點數由業務人工確認
  ...
  規則: 報價輸入條件
    Example: 每週練習次數範圍限制
      Given 報價計算機啟用
      When 使用者輸入每週練習次數
      Then 系統僅允許 1~7 次/週，預設值為 3 次
      And 超出範圍則顯示錯誤訊息

    Example: 每次練習句數範圍限制
      Given 報價計算機啟用
      When 使用者輸入每次練習句數
      Then 系統預設 15 句，允許任意正整數
      And 建議範圍 5~30 句，超出時顯示提示
  ...
  規則: 報價結果儲存與分享
    Example: 產生報價連結供後續查詢
      Given 報價計算機啟用
      When 使用者點選「儲存報價」
      Then 系統要求填寫聯絡方式（姓名、電話、mail、方便連絡時間）
      And 完成後產生唯一報價連結
      And 使用者可透過連結查詢、分享給機構決策人
      And 業務可依連結與使用者聯絡

# 全文術語正規化：教師授權
# 將「教師名額」全部統一為「教師授權」，並於表格、描述、欄位、註解中一致使用
