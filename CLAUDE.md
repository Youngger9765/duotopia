...
# 使用絕對路徑避免混淆
git add /Users/young/project/duotopia/backend/main.py  # ✅ 明確
git add main.py  # ❌ 容易混淆

# 切換目錄時明確說明
cd /Users/young/project/duotopia  # 回到專案根目錄
cd backend  # 進入後端目錄
```
...
2. **優先使用絕對路徑**（從根目錄開始）
3. **在每個指令區塊開頭標註工作目錄**
4. **使用 `realpath` 顯示檔案完整路徑**

### 📂 專案目錄結構提醒：
```
/Users/young/project/duotopia/        # 專案根目錄（git 操作在這）
├── frontend/                         # 前端程式碼
├── backend/                          # 後端程式碼
│   ├── main.py                      # ⚠️ 在 backend/ 內，不是根目錄！
│   ├── alembic/                     # 資料庫 migrations
│   └── tests/                       # 測試檔案
└── .pre-commit-config.yaml           # 在根目錄
```
